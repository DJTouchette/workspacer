...
test('legacy Intent preference leaves ordinary Fleet navigation available after reload', async ({
  ...
  await page.evaluate(async () => {
    await (window as any).electronAPI.saveConfig({ ui: { intentWorkspaces: true } });
    (window as any).firstUse.restart();
  });
  await page.reload();
  await expect(page.getByLabel('Ask the Fleet Manager')).toBeVisible();
  ...
  await expect(page.getByRole('checkbox', { name: 'Intent workspaces (preview)' })).toHaveCount(0);
});
