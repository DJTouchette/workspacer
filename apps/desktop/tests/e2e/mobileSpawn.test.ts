...
test('browse remote folders and dispatch an explicit model, effort, full access and edited task', async ({
  page,
}, testInfo) => {
      ...
      if (f.op === 'call' && f.method === 'fs.listDir') {
        ...
        route.send(
          JSON.stringify({
            ...
            id: f.id,
            result: {
              path: selected ? '/data/repos/mobile-demo' : '/data/repos',
              parent: '/data/repos',
              home: '/data/home',
              dirs: selected ? [] : ['mobile-demo'],
            },
          }),
        );
...
test('changing providers clears an incompatible model and preserves the drafted task', async ({
  page,
}) => {
  await page.goto(`${hub.url}/m?token=${HOST_TOKEN}`);
  await page.locator('nav [data-tab="new"]').click();
  ...
  expect(hub.callsTo('agents.spawn')[0].params).toMatchObject({
    provider: 'codex',
    model: 'gpt-5.4',
    message: 'My task survives changing provider.',
  });
