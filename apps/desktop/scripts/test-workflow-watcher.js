...
workflowWatcher.attach('test-session', transcript, (update) => {
  ...
  for (const run of update.runs) {
    console.log(
      `run ${run.runId}  name=${run.name}  status=${run.status}  agents=${run.agents.length}  phases=[${run.phases.map((p) => p.title).join(', ')}]  tokens=${run.totalTokens ?? '-'}  dur=${run.durationMs ?? '-'}ms`,
    );
    for (const a of run.agents) {
      console.log(
        `  agent ${a.id}  [${a.status}]  label=${a.label ?? '-'}  phase=${a.phaseTitle ?? '-'}  model=${a.model ?? '-'}  tok=${a.tokens}  tools=${a.toolCalls}  lastTool=${a.lastToolName ?? '-'}(${a.lastToolSummary ?? ''})  prompt=${(a.promptPreview ?? '').slice(0, 50)}`,
      );
    }
  }
  ...
  for (const id of plainIds.slice(0, 5)) {
    ...
    console.log(
      `  ${id}  type=${a.agentType ?? '-'}  desc=${(a.description ?? '').slice(0, 50)}  tok=${a.tokens}  tools=${a.toolCalls}  lastTool=${a.lastToolName ?? '-'}(${a.lastToolSummary ?? ''})`,
    );
  }
