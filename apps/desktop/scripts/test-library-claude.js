...
check(
  'skill listed with scope=claude',
  !!skill && skill.id === 'deploy-checklist' && skill.title === 'deploy-checklist',
);
...
check(
  'agent listed with scope=claude',
  !!agent && agent.id === 'reviewer' && agent.kind === 'agent',
);
...
libraryService.save({
  ...
  title: 'reviewer',
  kind: 'agent',
  description: 'Reviews PRs thoroughly',
  body: 'You are a strict but kind code reviewer.',
  cwd: root,
});
...
libraryService.save({
  ...
  kind: 'skill',
  description: 'Steps before deploying (updated)',
  body: 'Run smoke tests, then check the dashboard.',
  cwd: root,
});
...
check(
  'new skill uses name frontmatter',
  fs.readFileSync(newSkill, 'utf-8').startsWith('---\nname: New Release Notes'),
);
...
check(
  '.workspacer library dir is still auto-created (existing behavior)',
  fs.existsSync(path.join(bare, '.workspacer', 'library')),
);
