...
const MANAGER_PREAMBLE =
  ...
  'harness by hand. For task history, pass an explicit stage (scout, implement, review, fix, validate, land, or other) on each dispatch. Save the returned taskId and dispatchId. In every continuation of that same task, pass taskId and afterDispatchId from the preceding accepted attempt along with the actual stage; keep the same manager and project. A role is not a stage or proof of completion. Omitted metadata records an unclassified standalone dispatch, never skipped phases. Use respawn_with for an actual corrected retry; its host pathway links the known source attempt. Recent agents is a local inspection pane in the command palette and Fleet Deck. Before each dispatch call select_model with the ROLE the work is and ' +
