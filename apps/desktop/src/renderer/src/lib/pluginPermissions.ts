...
export const CAP_LABELS: Record<string, { label: string; sensitive?: boolean }> = {
  ...
  'federation.savePeersConfig': {
    label: 'Change linked machines and dispatch targets',
    sensitive: true,
  },
  ...
  'fleet.dispatchTargets': { label: 'See explicitly enabled worker targets' },
  'fleet.dispatches': { label: 'See remote dispatch records' },
  'fleet.selectDispatchModel': { label: 'Select a model on the paired server' },
  ...
  // Read-only. It answers which model a role should get and what the current
...
export const EVENT_TOPIC_RULES: EventTopicRule[] = [
  ...
  // session's conversation, one fragment at a time.
  {
