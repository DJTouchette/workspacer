...
export function browserAssets(
  ...
  const asset = (kind: 'font' | 'icon', file: string) =>
    call<Asset>('ui.asset', { kind, file }, 90_000);
  const load = (font: Font, replace = false): Promise<FontFace> => {
    if (replace) {
      fonts.delete(font.file);
      const old = fontFaces.get(font.file);
    ...
    if (!loading) {
      loading = asset('font', font.file).then(async (data) => {
        ...
        const face = new FontFace(JSON.stringify(font.family), bytes.buffer, {
          weight: '100 900',
          display: 'swap',
        });
        await face.load();
        document.fonts.add(face);
        fontFaces.set(font.file, face);
        return face;
      });
  ...
  const api: Pick<
    ElectronAPI,
    'getUiAsset' | 'listUiFonts' | 'installUiFont' | 'downloadProjectIcon'
  > = {
    getUiAsset: asset,
    listUiFonts: list,
    installUiFont: async () => {
      ...
      const installed = await call<Font>(
        'desktop.installUiFont',
        { name: file.name, dataBase64: await readFileBase64(file) },
        90_000,
      );
    ...
    downloadProjectIcon: async (url) => {
      try {
        return await call('desktop.downloadProjectIcon', { url });
      } catch (error) {
        return {
          ok: false,
          error: error instanceof Error ? error.message : 'Icon download failed',
        };
      }
    },
  };
