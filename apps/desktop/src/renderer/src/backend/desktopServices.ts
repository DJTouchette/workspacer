...
export function desktopServices(
  ...
  return {
    ...
    providerReadiness: (provider, check) =>
      call('desktop.providerReadiness', { provider, check }, 60_000),
    ...
    claudeProfilesAdd: (name, configDir, extraArgs, mcpItemIds, init) =>
      call('desktop.claudeProfilesAdd', { name, configDir, extraArgs, mcpItemIds, init }),
    ...
    managerRequestPrepare: (sessionId, text, bootstrap) =>
      call('desktop.managerRequestPrepare', { sessionId, text, bootstrap }),
    ...
    taskInspectorOpen: async (request) => {
      try {
        const result = await call<{
          ...
          target?: string;
          error?: string;
        }>('desktop.taskInspectorOpen', { request });
        if (!result.ok || !result.target)
          return { ok: false, error: result.error ?? 'Target unavailable' };
        ...
        return {
          ok: false,
          error: error instanceof Error ? error.message : 'Could not open target',
        };
      }
    },
