...
const LayoutsDialog: React.FC<Props> = ({ agentCount, onSaveCurrent, onRestore, onClose }) => {
  ...
  const reload = () => {
    window.electronAPI
      .layoutsList()
      .then((l) => setLayouts(Array.isArray(l) ? l : []))
      .catch(() => {});
  };
  useEffect(() => {
    reload();
      ...
      <div onClick={(e) => e.stopPropagation()} style={modal}>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
          <input
            ...
            placeholder={
              agentCount > 0
                ? `Save ${agentCount} agent${agentCount === 1 ? '' : 's'} as…`
                : 'No agents to save'
            }
            ...
            autoFocus
          />
          <button onClick={save} disabled={agentCount === 0 || !name.trim()} style={primaryBtn}>
            Save current
          </button>
        </div>
            ...
            layouts.map((l) => (
              ...
              >
                ...
                >
                  ...
                  >
                    {l.agents.length} {l.agents.length === 1 ? 'dir' : 'dirs'}:{' '}
                ...
                <button
                  ...
                  style={miniBtn}
                  title="Restore — spawn fresh agents"
                >
                  Restore
                </button>
                <button
