...
const LibrarySidePanel: React.FC<Props> = ({ visible, onClose, cwd }) => {
  ...
  return (
    <div
      aria-hidden={!visible}
      style={{
        position: 'fixed', top: 0, right: 0, bottom: 0, width: PANEL_W, maxWidth: '92vw',
        zIndex: 1800,
    ...
    >
      ...
      {/* Search + scope filter */}
      <div style={{ padding: '10px 14px', display: 'flex', flexDirection: 'column', gap: 8, borderBottom: '1px solid var(--wks-glass-border)' }}>
        <input
          value={query}
          ...
          autoFocus
          style={{
            height: 30, padding: '0 10px', fontSize: '0.78rem', fontFamily: 'inherit',
            color: 'var(--wks-text-primary)', background: 'var(--wks-bg-base)',
            border: '1px solid var(--wks-border-input)', borderRadius: 6, outline: 'none',
          }}
        />
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: 12, display: 'flex', flexDirection: 'column', gap: 8 }}>
        {filtered.length === 0 && (
          <div style={{ textAlign: 'center', color: 'var(--wks-text-faint)', padding: 36, fontSize: '0.76rem' }}>
        ...
        {filtered.map((it) => (
          <div key={`${it.scope}:${it.id}`} style={card(flashId === it.id)}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              <span style={{ fontWeight: 600, fontSize: '0.78rem', color: 'var(--wks-text-primary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{it.title}</span>
              <span style={kindBadge(it.kind)}>{it.kind}</span>
            </div>
            {it.description && (
              <div style={{ fontSize: '0.66rem', color: 'var(--wks-text-secondary)', marginTop: 3 }}>{it.description}</div>
            )}
            <div style={{ fontSize: '0.62rem', color: 'var(--wks-text-faint)', marginTop: 4, maxHeight: 30, overflow: 'hidden', fontFamily: 'var(--wks-mono, ui-monospace, monospace)', lineHeight: 1.4 }}>
              {it.body.replace(/\s+/g, ' ').slice(0, 140)}
            </div>
            <div style={{ display: 'flex', gap: 6, marginTop: 8 }}>
              <button onClick={() => sendToAgent(it)} style={primaryBtn} title="Insert into the active agent's input">
                {flashId === it.id ? '✓ Added' : '→ Agent'}
              </button>
              <button onClick={() => runLibraryItem(it, 'copy')} style={ghostBtn} title="Copy to clipboard">Copy</button>
            </div>
          </div>
        ))}
      </div>
...
const closeBtn: React.CSSProperties = {
  ...
  fontSize: '0.85rem', color: 'var(--wks-text-faint)', background: 'transparent',
  border: 'none', borderRadius: 5, cursor: 'pointer',
};

const chip = (active: boolean): React.CSSProperties => ({
  flex: 1, padding: '4px 0', fontSize: '0.62rem', fontWeight: 600, textTransform: 'capitalize',
...
const kindBadge = (kind: LibraryKind): React.CSSProperties => ({
  ...
  padding: '1px 6px', borderRadius: 999, color: 'var(--wks-text-muted)',
  background: 'var(--wks-bg-elevated)', flexShrink: 0,
});

const primaryBtn: React.CSSProperties = {
  flex: 1, height: 26, fontSize: '0.7rem', fontWeight: 600, fontFamily: 'inherit',
  color: '#fff', background: 'var(--wks-accent)', border: '1px solid var(--wks-accent)',
  borderRadius: 6, cursor: 'pointer',
};

const ghostBtn: React.CSSProperties = {
  flex: '0 0 auto', height: 26, padding: '0 12px', fontSize: '0.7rem', fontWeight: 600, fontFamily: 'inherit',
  color: 'var(--wks-text-secondary)', background: 'var(--wks-bg-elevated)',
  border: '1px solid var(--wks-border-input)', borderRadius: 6, cursor: 'pointer',
};
