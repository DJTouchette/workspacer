...
const SpawnAgentDialog: React.FC<SpawnAgentDialogProps> = ({
  ...
  const runtimeStatus = useAgentRuntimeStatus(provider !== 'pi', !!targetHub);
  ...
  // Installed plugins can contribute launch integrations. Agent-facing plugin
  // tools are ambient through the authenticated Workspacer facade, so there is
  // no per-spawn grant picker here.
  useEffect(() => {
    let cancelled = false;
    window.electronAPI
      .listHubPlugins?.()
