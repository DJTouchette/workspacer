...
export const WorkflowTimeline: React.FC<{
  ...
  useEffect(() => {
    ...
    setTranscript(null);
    window.electronAPI
      .workflowAgentTranscript(sessionId, txRunId, selectedId)
      .then((t) => {
  ...
  const { lanes, phaseDetail } = useMemo(() => {
    ...
    for (const a of run.agents) {
      ...
      if (!byTitle.has(a.phaseTitle)) {
        byTitle.set(a.phaseTitle, []);
        order.push(a.phaseTitle);
      }
      byTitle.get(a.phaseTitle)!.push(a);
    }
    const out: { title: string | null; agents: WorkflowAgentInfo[] }[] = order.map((t) => ({
      title: t,
      agents: byTitle.get(t)!,
    }));
  ...
  const card = (
    <div onClick={embedded ? undefined : (e) => e.stopPropagation()} style={cardStyle}>
      {/* Header */}
      <div
        style={{
          display: 'flex',
      ...
      >
        <span style={{ color: AGENT_PURPLE, fontWeight: 700, fontSize: '0.9rem' }}>
          {heading ?? 'Workflow'}
        </span>
        <span
          style={{
        ...
        <span style={metaStyle}>
          {finished}/{run.agents.length} agents
        </span>
        {failed > 0 && (
          <span style={{ ...metaStyle, color: colors.error, fontWeight: 700 }}>
            {failed} failed
          </span>
        )}
            ...
            <div key={`${lane.title ?? 'ungrouped'}-${li}`} style={{ marginBottom: 14 }}>
              ...
              {lane.agents.map((a) => {
                ...
                const widthPct = Math.max(
                  1.5,
                  Math.min(100 - leftPct, ((aEnd - aStart) / span) * 100),
                );
                const dur =
                  a.durationMs ??
                  (a.status === 'running' && a.startedAt ? now - a.startedAt : undefined);
                  ...
                  >
                    ...
                    <span
                      style={{
                        ...
                        flexShrink: 0,
                        fontSize: '0.72rem',
                        color: sel ? colors.textBright : colors.text,
                        overflow: 'hidden',
                        textOverflow: 'ellipsis',
                        whiteSpace: 'nowrap',
                      }}
                    ...
                    <span style={{ ...metaStyle, width: 96, textAlign: 'right' }}>
                      {a.tokens > 0 ? `${fmtTokens(a.tokens)} · ` : ''}
                      {dur !== undefined ? fmtDuration(dur) : ''}
                    </span>
                  </div>
                );
              })}
        ...
        {selected && (
          ...
          >
            ...
            {selected.resultPreview && (
              <div>
                ...
                <div
                  style={{
                    ...detailBody,
                    color: selected.status === 'failed' ? colors.error : colors.text,
                  }}
                >
                  {selected.resultPreview}
                </div>
            ...
            {!selected.promptPreview && !selected.resultPreview && (
              <div style={{ color: colors.mutedDim }}>
                No prompt/result captured{selected.status === 'running' ? ' yet' : ''}.
              </div>
            )}
            ...
            >
              ...
              {!loadingTx && (transcript?.length ?? 0) === 0 && (
                <div style={{ color: colors.mutedDim }}>
                  {transcript === null ? 'Transcript unavailable.' : 'No messages yet.'}
                </div>
              )}
              {!loadingTx &&
                transcript &&
