  ...
  return (
    <div style={{ padding: '2px 0' }}>
      ...
      >
        ...
        <div style={{ flex: 1 }} />
        <span style={agentMetaStyle}>
          {done}/{agents.length}
        </span>
        {failed > 0 && (
          <span style={{ ...agentMetaStyle, color: colors.error, fontWeight: 600 }}>
            {failed} failed
          </span>
        )}
      ...
      {expanded && (
        <div style={{ paddingLeft: 8 }}>
          ...
          {agents.map((a) => (
            <WorkflowAgentRow key={a.id} agent={a} now={now} />
          ))}
        </div>
      )}
    </div>
          ...
          {groups.map((g, gi) =>
            g.title ? (
              <PhaseGroup
                key={`${g.title}-${gi}`}
                title={g.title}
                detail={phaseDetail.get(g.title)}
                agents={g.agents}
                now={now}
              />
            ) : (
