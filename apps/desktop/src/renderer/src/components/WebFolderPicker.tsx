...
const WebFolderPicker: React.FC = () => {
  ...
  const browse = useCallback((path?: string) => {
    setError(null);
    window.electronAPI.fsListDir?.(path)
      .then((l) => setListing(l))
  ...
  return (
    ...
    >
      <div
        ...
        style={{
          width: 460, maxWidth: '94vw', maxHeight: '80vh', display: 'flex', flexDirection: 'column',
          ...
          padding: 16, boxShadow: '0 16px 48px var(--wks-glass-shadow)', fontFamily: 'inherit',
        }}
      >
        ...
        <div style={{ display: 'flex', gap: 6, alignItems: 'center', marginBottom: 8 }}>
          <button title="Home" onClick={() => browse(listing?.home)} style={iconBtn}><Home size={13} /></button>
          <button title="Up one level" onClick={() => listing && browse(listing.parent)} style={iconBtn}><ArrowUp size={13} /></button>
          <div style={{
            flex: 1, minWidth: 0, fontSize: '0.72rem', fontFamily: 'var(--wks-font-mono, monospace)',
            color: 'var(--wks-text-tertiary)', background: 'var(--wks-bg-base)',
        ...
        {/* Directory list */}
        <div style={{
          flex: 1, overflowY: 'auto', minHeight: 160, maxHeight: '46vh',
          border: '1px solid var(--wks-border-subtle)', borderRadius: 6, padding: 4,
        }}>
          {error && <div style={{ padding: 10, fontSize: '0.72rem', color: 'var(--wks-danger, #e05555)' }}>{error}</div>}
          {!error && listing && listing.dirs.length === 0 && (
            <div style={{ padding: 10, fontSize: '0.72rem', color: 'var(--wks-text-faint)' }}>No subfolders here.</div>
          )}
          {!error && listing?.dirs.map((name) => (
            <button
              key={name}
              onClick={() => browse(joinPath(listing.path, name))}
              style={{
                display: 'flex', alignItems: 'center', gap: 8, width: '100%', textAlign: 'left',
                padding: '6px 8px', border: 'none', borderRadius: 4, cursor: 'pointer',
                background: 'transparent', color: 'var(--wks-text-secondary)', fontSize: '0.76rem', fontFamily: 'inherit',
              }}
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8, marginTop: 12 }}>
          <button onClick={() => finish(null)} style={secondaryBtn}><X size={13} /> Cancel</button>
          <button onClick={() => finish(listing?.path || null)} disabled={!listing} style={primaryBtn(!listing)}>
...
const secondaryBtn: React.CSSProperties = {
  display: 'inline-flex', alignItems: 'center', gap: 5, fontSize: '0.76rem', fontFamily: 'inherit', cursor: 'pointer',
  background: 'transparent', color: 'var(--wks-text-tertiary)',
  border: '1px solid var(--wks-border-input)', borderRadius: 4, padding: '6px 12px',
};
const primaryBtn = (disabled: boolean): React.CSSProperties => ({
  display: 'inline-flex', alignItems: 'center', gap: 5, fontSize: '0.76rem', fontFamily: 'inherit',
