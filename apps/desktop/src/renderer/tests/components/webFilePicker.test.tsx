...
it('selects server file paths without invoking a local upload picker', async () => {
  ...
  window.electronAPI = {
    fsListDir: vi
      .fn()
      .mockResolvedValue({ path: '/server/project', home: '/server', parent: '/server', dirs: [] }),
    filePickerList: vi.fn().mockResolvedValue({
      path: '/server/project',
      parent: '/server',
      home: '/server',
      entries: [{ name: 'source.ts', path: '/server/project/source.ts', isDir: false }],
    }),
  } as any;
  render(<WebFolderPicker />);
  act(() =>
    window.dispatchEvent(
      new CustomEvent('web:pick-files', { detail: { defaultPath: '/server/project', resolve } }),
    ),
  );
...
it('settles the pending chooser when the user cancels', async () => {
  ...
  window.electronAPI = {
    fsListDir: vi
      .fn()
      .mockResolvedValue({ path: '/server', home: '/server', parent: '/', dirs: [] }),
  } as any;
  render(<WebFolderPicker />);
