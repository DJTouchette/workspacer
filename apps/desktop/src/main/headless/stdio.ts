...
input.on('line', (line) => {
  let request: {
    ...
    context: Parameters<typeof desktopHostCall>[2];
  };
  try {
  ...
  if (typeof request.id !== 'string' || typeof request.method !== 'string' || !request.context) {
    process.exitCode = 1;
    input.close();
    return;
  }
    ...
    .then(
      ...
      (error) =>
        process.stdout.write(
          JSON.stringify({
            id: request.id,
            error: error instanceof Error ? error.message : 'Desktop service failed',
          }) + '\n',
        ),
    )
    .finally(() => {
      active--;
      if (closed && active === 0) process.exit();
    });
});
input.on('close', () => {
  closed = true;
