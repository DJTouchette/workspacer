...
import {
  ensureReplacementReady,
  replacementRequest,
  rememberManager,
  routeReplacementMessage,
  finishReplacementMessage,
} from './managerReplacement';
...
type PendingAdmission = {
  input: Omit<Admission, 'sessionId'>;
...
export function setDesktopEventSink(sink: typeof emit): void {
  emit = sink;
}
...
export async function desktopHostCall(
  ...
  switch (method) {
    case 'internal.prepareIntegration': {
      if (!context.spawnCallId)
        throw new Error('Launch integration requires an active owner-authorized bus spawn');
      const pluginId = text(params, 'id');
      return prepareLaunchIntegrationCore(
        pluginId,
        params.launchContext as Parameters<typeof prepareLaunchIntegrationCore>[1],
        params.base as Parameters<typeof prepareLaunchIntegrationCore>[2],
        {
          list: () =>
            hostCall('launch.prepare', { callId: context.spawnCallId, pluginId, op: 'describe' }),
          call: (_method, launchContext) =>
            hostCall('launch.prepare', {
              callId: context.spawnCallId,
              pluginId,
              op: 'prepare',
    ...
    case 'desktop.managerReplacement':
      return replacementRequest(
        params.request as Parameters<typeof replacementRequest>[0],
        params.bindings,
      );
    case 'internal.rememberManager':
      rememberManager(
        text(params, 'sessionId'),
        params.options as Parameters<typeof rememberManager>[1],
      );
      return { ok: true };
    case 'internal.routeMessage':
      await ensureReplacementReady();
      return routeReplacementMessage(
        text(params, 'sessionId'),
        text(params, 'text'),
        params.sourceRequest as Parameters<typeof routeReplacementMessage>[2],
      );
    case 'internal.messageResult':
      finishReplacementMessage(text(params, 'id'), text(params, 'status'));
      return { ok: true };
    case 'internal.assertReplacementAvailable':
      managerReplacementState.assertAvailable(text(params, 'source'));
      managerReplacementState.assertAvailable(text(params, 'successor'));
      return { ok: true };
    ...
    case 'internal.analyticsRecent':
      ...
      return headlessAnalytics(
        method === 'internal.analyticsSummary' ? 'analytics.summary' : 'analytics.recent',
        params,
    ...
    case 'internal.prepareSpawn': {
      ...
      const run = managerReplacementState.admitted(
        [
          submitted.dispatchOwnerSessionId as string | undefined,
          submitted.parentSessionId as string | undefined,
        ],
        () =>
          workflow.workflowSpawn(async (wire) => {
            const p = wire as Params;
            const owner =
              typeof p.parentSessionId === 'string' &&
              p.dispatchOwnerSessionId === p.parentSessionId
                ? currentSnapshot(p.parentSessionId)
                : undefined;
            ...
            if (p.workflowStepId && p.taskId)
              template = workflow.pinnedWorkflowTemplate(
                String(p.taskId),
                String(p.workflowStepId),
              );
            else if (p.template) {
              if (typeof p.message === 'string' && p.message.trim())
                throw new Error('Pass template or message, not both');
              template = dispatchTemplates.find((t) => t.id === p.template);
              if (!template)
                throw new Error(`Dispatch template unavailable: ${String(p.template)}`);
            } else if (p.templateParams && Object.keys(p.templateParams).length)
              throw new Error('templateParams requires a template');
            if (template)
              validateDispatchTemplateParams(
                template.body,
                (p.templateParams ?? {}) as Record<string, string>,
              );
            const schema = (
              p.workflowStepId ? template?.resultSchema : (p.resultSchema ?? template?.resultSchema)
            ) as Record<string, unknown> | undefined;
            if (schema !== undefined) {
              const error = checkResultSchema(schema);
              if (error) throw new Error(error);
            }
            let review: ReviewAllocation | undefined;
            if (p.worktree === true) {
              const source = assertPathAllowed(
              ...
              const result = await createWorktree({
                ...
                rootOverride: configuredWorktreeRoot(),
              ...
              if (result.ok && result.path) {
                input.executionCwd = result.path;
                review = result.reviewAllocation;
              }
              if (p.workflowStepId && !result.ok)
                throw new Error('Workflow ship step requires successful worktree allocation');
            }
            const patch: Params = { cwd: input.executionCwd };
            if (template) {
              patch.message = renderDispatchTemplate(
                template.body,
                (p.templateParams ?? {}) as Record<string, string>,
                { cwd: input.executionCwd, projectCwd },
              );
              patch.resultSchema = p.workflowStepId
                ? template.resultSchema
                : (p.resultSchema ?? template.resultSchema);
            }
            ...
            prepared({
              ...
              patch,
              schema,
              contract: schema ? buildResultContract(schema) : '',
            });
            return completion;
          })(submitted),
      );
    ...
    case 'internal.acceptSpawn': {
      ...
      try {
        const accepted = admission.resume
          ? undefined
          : dispatchHistoryStore.accept({
              ...admission.input,
              owner: admission.input.owner
                ? currentSnapshot(admission.input.owner.sessionId)
                : undefined,
              sessionId,
            });
        if (admission.review && admission.input.owner?.isWakeTarget)
          fleetReviewStore.register(admission.input.owner.sessionId, sessionId, admission.review);
        managerReplacementState.rememberChild({
          sessionId,
          cwd: admission.input.executionCwd,
          label: admission.input.title,
          parentSessionId: admission.input.owner?.sessionId,
          provider: admission.input.provider,
        });
    ...
    case 'internal.workflowRequest':
      return workflows.fleetWorkflowRequest(
        params as unknown as WorkflowRequest,
        text(params, 'callerSessionId', true) || undefined,
      );
    ...
    case 'internal.beginDelivery': {
      const delivery = managerRequests().beginDelivery(
        managerReplacementState.wakeTarget(text(params, 'sessionId')),
        text(params, 'requestId'),
      );
      if (!delivery) return null;
      return {
        ...delivery,
        text: delivery.bootstrap ? buildManagerKickoff(delivery.text) : delivery.text,
      };
    }
    case 'internal.finishDelivery':
      managerRequests().finishDelivery(
        text(params, 'requestId'),
        text(params, 'deliveryId'),
        params.status as 'accepted' | 'rejected' | 'unknown',
      );
      return { ok: true };
    case 'internal.requestReceipt': {
      const request = managerRequests().request(
        managerReplacementState.wakeTarget(text(params, 'sessionId')),
        text(params, 'requestId'),
      );
      return {
        ok: request.delivery === 'accepted' || request.delivery === 'pending',
    ...
    case 'internal.finishWorker': {
      ...
      const task = dispatchHistoryStore
        .list()
        .find((t) => t.attempts.some((a) => a.sessionId === sessionId));
      ...
      const evidenceId = await fleetReviewStore.capture(
        task.ownerSessionId,
        sessionId,
        session.status === 'ended' ? 'session-ended' : 'turn-ended',
      );
      const current = currentSnapshot(sessionId);
      if (!current || (current.status !== 'ended' && current.ambientState !== 'idle')) return {};
      const token = randomUUID();
      for (const [id, pending] of resultCommits)
        if (pending.expires < Date.now()) resultCommits.delete(id);
      if (resultCommits.size >= 128) throw new Error('Too many pending result validations');
      const fingerprint = (s: typeof current | undefined) =>
        JSON.stringify([s?.status, s?.ambientState, s?.lastActivity, s?.cwd, s?.liveCwd]);
      const expected = fingerprint(current);
      resultCommits.set(token, {
        expires: Date.now() + 60_000,
        commit: () => {
          if (fingerprint(currentSnapshot(sessionId)) !== expected)
            throw new Error('Worker changed during result capture');
            ...
            escalation?.json
              ...
              : result.json
                ...
                : result.error
                  ...
                  : 'absent',
            evidenceId,
            result.json ? JSON.parse(extractResultBlock(reply)!) : escalation?.json,
          );
          return {
            result: result.json,
            resultError: result.error,
            reviewEvidenceId: evidenceId,
            instructions: workflow.workflowWakeInstructions([sessionId]),
          };
        },
      });
      return { token };
    ...
    case 'internal.commitWorkerResult': {
      const token = text(params, 'token'),
        pending = resultCommits.get(token);
      resultCommits.delete(token);
    ...
    case 'internal.observe': {
      ...
      for (const s of context.snapshots) {
        if (watchWorkflow(s.sessionId) && s.status !== 'ended' && s.ambientState !== 'idle')
          workflowWatcher.poke(s.sessionId);
      }
      for (const id of workflowWatching.keys())
        if (!context.snapshots.some((s) => s.sessionId === id)) {
          workflowWatcher.detach(id);
          workflowWatching.delete(id);
          workflowTelemetry.forgetSession(id);
        }
