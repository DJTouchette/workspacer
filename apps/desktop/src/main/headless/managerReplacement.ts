...
import {
  managerReplacementState as state,
  type ManagerLaunch,
...
const inFlight = new Map<
  string,
  { id: string; target: string; text: string; sourceRequest?: ReplacementDelivery['sourceRequest'] }
>();
let ready = false;
...
function source(id: string, paneId?: string): ManagerLaunch {
  ...
  if (
    ...
    !recorded ||
    !grants ||
    (paneId && !managerViewerBound(paneId, id))
  )
    throw new Error('Manager launch, live operator identity, and attached viewer are required');
  ...
  if (recorded.configuration !== managerLaunchConfiguration(recorded.options))
    throw new Error(
      'Manager profile configuration changed; replacement cannot reproduce this launch',
    );
  const model =
    row.requestedSelection?.model ||
    row.settings?.model ||
    recorded.options.model ||
    row.usage?.model;
  if (!model)
    throw new Error('Manager model is not recorded yet; wait for its first configured turn');
  const selection = normalizeModelSelection(
    model,
    row.requestedSelection
      ? row.requestedSelection.contextWindow
      : row.settings?.contextWindow !== undefined
        ? row.settings.contextWindow
        : recorded.options.contextWindow,
  );
  const permissionMode =
    row.livePermissionMode ?? row.settings?.permissionMode ?? recorded.options.permissionMode;
  return {
    ...recorded,
    grants,
    options: {
      ...recorded.options,
      cwd: row.cwd,
      label: row.label ?? recorded.options.label,
      ...
      effort: row.liveEffort ?? row.statusLine?.effort ?? recorded.options.effort,
      permissionMode,
      skipPermissions: permissionMode === 'yolo' || permissionMode === 'bypassPermissions',
    },
  };
}
function inventory(id: string): ReplacementMetadata[] {
  return snapshots
    .filter((s) => !s.hub && (s.sessionId === id || s.parentSessionId === id))
    .map((s) => ({
      sessionId: s.sessionId,
      cwd: s.cwd,
      label: s.label,
      parentSessionId: s.parentSessionId,
      isWakeTarget: s.isWakeTarget,
      provider: s.provider,
...
const service = new ManagerReplacementService(state, {
  ...
  async spawn(id, launch) {
    await hostCall('replacement.spawn', {
      sessionId: id,
      sourceSessionId: state.related(id)?.sourceSessionId,
      launch,
    });
    await refresh();
  },
  async validateSuccessor(id, launch) {
    await refresh();
    const s = snapshot(id),
      actual = state.launch(id)?.options,
      op = state.related(id);
    if (
      !s ||
      s.status === 'ended' ||
    ...
    if (
      ...
      throw new Error('Successor launch selection changed');
  },
  async transfer(oldId, newId, operationId) {
    const op = state.get(operationId);
    if (op.sourceSessionId !== oldId || op.successorSessionId !== newId || !op.transferIntent)
      throw new Error('Missing durable ownership intent');
    ...
    await hostCall('replacement.transfer', {
      source: oldId,
      successor: newId,
  ...
  async restore(metadata) {
    await hostCall('replacement.restore', { metadata });
    await refresh();
  },
  ...
  pause: async (id) => {
    await hostCall('replacement.signal', { sessionId: id, signal: 'SIGINT' });
  },
  close: async (id) => {
    await hostCall('replacement.close', { sessionId: id });
  },
  kickoff: (op) =>
    buildManagerKickoff(
  ...
  async flushFinishes(ids) {
    ...
    while ([...inFlight.values()].some((m) => ids.includes(m.target))) {
      if (Date.now() > deadline) throw new Error('Message acknowledgements are still in flight');
...
export async function ensureReplacementReady(): Promise<void> {
  ...
  if (state.recoveryMetadata().length) {
    await refresh();
...
export function rememberManager(sessionId: string, options: ManagerLaunch['options']) {
  ...
  state.rememberLaunch(sessionId, {
    options,
    grants,
    configuration: managerLaunchConfiguration(options),
  });
}
export function routeReplacementMessage(
  sessionId: string,
  text: string,
  sourceRequest?: ReplacementDelivery['sourceRequest'],
) {
  // Initialize recovery before messages can move through a saved lineage.
  if (!ready && state.records().length)
    throw new Error('Manager recovery must be reconciled before message delivery');
  const target = state.wakeTarget(sessionId);
  if (state.holdMessage(target, text, [], sourceRequest)) return { target, held: true };
  const id = randomUUID();
  inFlight.set(id, { id, target, text, sourceRequest });
  return { target, id, held: false };
}
export function finishReplacementMessage(id: string, status: string) {
  const entry = inFlight.get(id);
  if (!entry) return;
  state.noteInFlightMessage(
    entry.target,
    id,
    status === 'accepted',
    status === 'accepted' ? undefined : 'Daemon acknowledgement ' + status,
    status === 'rejected',
  );
  inFlight.delete(id);
}
