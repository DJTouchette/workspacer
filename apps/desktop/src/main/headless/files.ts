...
export function readFileBytes(requested: unknown, roots: string[]) {
  ...
  try {
    const inspected = fs.fstatSync(fd),
      current = fs.statSync(file);
    if (
      !inspected.isFile() ||
      inspected.size > limit ||
      inspected.ino !== current.ino ||
      inspected.dev !== current.dev ||
      assertPathAllowed('desktop.readFileBytes', file, roots) !== file
    )
      throw new Error('Choose an unchanged regular file up to 16 MiB');
    const bytes = Buffer.alloc(limit + 1);
    let size = 0;
    while (size < bytes.length) {
      const n = fs.readSync(fd, bytes, size, bytes.length - size, null);
      if (!n) break;
      size += n;
    }
...
export function listPickerEntries(requested: unknown) {
  ...
  if (entries.length > 10_000)
    throw new Error('Directory has too many entries; choose a narrower path');
  const files = entries
    .map((entry) => {
