...
export function hostCall<T = any>(method: string, params: unknown = {}): Promise<T> {
  ...
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      pending.delete(hostCallId);
      reject(new Error('Lifecycle acknowledgement timed out; outcome may be unknown'));
    }, 60_000);
...
export function acceptHostResult(frame: any): boolean {
  ...
  if (entry) {
    pending.delete(frame.hostResultId);
    clearTimeout(entry.timer);
    frame.error ? entry.reject(new Error(frame.error)) : entry.resolve(frame.result);
  }
  return true;
}
