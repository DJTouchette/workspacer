...
async function open(): Promise<void> {
  ...
  opening = (async () => {
    ...
    history = new SessionHistoryStore(
      ...
      } as unknown as { db: Pick<BetterSqlite3.Database, 'prepare'> },
      true,
    );
  })();
  try {
  ...
  for (const root of roots) {
    ...
    for (const dir of dirs) {
      ...
      try {
        ...
        /* A concurrently removed project is absent. */
      }
    }
  }
  return out;
...
async function observe(rows: Row[]): Promise<void> {
  ...
  for (const row of rows) {
    ...
    const model =
      usage.model || sl.modelDisplay || sl.model_display || sl.model || prior?.model || '';
    ...
    const record: SessionHistoryRecord = {
      sessionId: id,
      cwd: row.cwd ?? '',
      agentName: row.label || path.basename(row.cwd || '') || id.slice(0, 8),
      provider,
      model,
      gitBranch: sl.gitBranch ?? sl.git_branch ?? String(prior?.git_branch ?? ''),
      startedAt: start,
      endedAt: end,
      durationMs: start
        ? Math.max(
            0,
            (stopped ? Date.parse(end) || Date.parse(start) : Date.now()) - Date.parse(start),
          )
        : 0,
      inputTokens: number(
      ...
      outputTokens: number(
        ...
        sl.totalOutputTokens,
        sl.total_output_tokens,
        prior?.output_tokens,
      ),
      costUSD:
        provider === 'claude'
          ? number(prior?.cost_usd, usage.costUSD, usage.cost_usd, sl.costUSD, sl.cost_usd)
          : number(sl.costUSD, sl.cost_usd, prior?.cost_usd),
      peakContext: Math.max(
        number(row.peakContext, usage.contextTokens, usage.context_tokens),
        number(prior?.peak_context),
      ),
      toolCalls: number(row.totalToolCalls, row.tool_calls, prior?.tool_calls),
      messageCount: number(row.messageCount, prior?.message_count),
      subagentCount:
        subagents.length + workflows.reduce((n: number, w: Row) => n + (w.agents?.length ?? 0), 0),
      workflowRuns: workflows.length || number(prior?.workflow_runs),
      workflowFailed: workflows.filter((w: Row) => w.status === 'failed').length,
      status: stopped ? 'ended' : 'active',
    };
    let models:
      Record<string, { inputTokens: number; outputTokens: number; costUSD: number }> | undefined;
    ...
    if (provider === 'claude') {
      ...
      if (typeof candidate === 'string') {
        ...
        const files = [
          main,
          ...subagentFilesFor(main).map((file) =>
            assertPathAllowed('analytics.summary', file, roots),
          ),
        ];
        try {
          fingerprint = JSON.stringify(
            files.map((file) => {
              ...
              return [file, s.dev, s.ino, s.size, s.mtimeMs];
            }),
          );
          const previous = db
            .prepare('SELECT fingerprint FROM analytics_sources WHERE session_id=?')
            .get(id);
          if (fingerprint !== previous?.fingerprint) {
            ...
            record.inputTokens = Number(prior.input_tokens);
            record.outputTokens = Number(prior.output_tokens);
            record.costUSD = Number(prior.cost_usd);
            record.model = String(prior.model);
            record.peakContext = Number(prior.peak_context);
          }
        } catch (error) {
    ...
    try {
      ...
      if (fingerprint)
        db.prepare('INSERT OR REPLACE INTO analytics_sources VALUES (?,?)').run(id, fingerprint);
      db.exec('COMMIT');
    } catch (error) {
      db.exec('ROLLBACK');
      throw error;
