...
function basename(value: unknown): string {
  if (
    ...
    throw new Error('Asset file must be one filename');
  return value;
}
export function listUIFonts() {
  try {
    return fs
      ...
      .map((file) => ({ file, family: customFontFamily(file) }));
  } catch (error) {
...
export function readUIAsset(kind: unknown, file: unknown) {
  ...
  if (
    kind === 'font'
      ? !FONT_EXT.test(name)
      : !/^[a-f0-9]{32}\.(png|jpg|gif|webp|svg|ico|avif)$/.test(name)
  )
  ...
  try {
    ...
    return {
      ...
      mime:
        kind === 'icon' ? mimeForIcon(name) : `font/${path.extname(name).slice(1).toLowerCase()}`,
      ...(kind === 'font' ? { family: customFontFamily(name) } : {}),
    };
  } finally {
    fs.closeSync(fd);
  }
...
export function installUIFont(name: unknown, dataBase64: unknown) {
  ...
  if (
    ...
    dataBase64.length > Math.ceil((FONT_LIMIT * 4) / 3) + 4
  )
    throw new Error('Choose a font file up to 12 MiB');
  const bytes = Buffer.from(dataBase64, 'base64');
  if (
    !bytes.length ||
    bytes.length > FONT_LIMIT ||
    !['00010000', '4f54544f', '74727565', '74797031', '774f4646', '774f4632'].includes(
