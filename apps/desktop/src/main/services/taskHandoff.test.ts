...
describe('workspace handoff host orchestration', () => {
  it('verifies preparation after all bytes and never substitutes remote HEAD', async () => {
    const calls: string[] = [];
    const endpoint =
      (side: string): HandoffCall =>
      async <T>(_method: string, raw: unknown) => {
  ...
  it('does not acknowledge custody for a dirty result', async () => {
    const local: HandoffCall = async () => {
      throw new Error('must not import dirty output');
    };
    const peer: HandoffCall = async <T>() => ({ ...record, state: 'needs-checkpoint' }) as T;
    expect(
      (await importTaskHandoffResult(manifest.task, record.plan.binding, local, peer)).state,
    ).toBe('needs-checkpoint');
  });
