...
function fixture() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(),'paired-registry-')); dirs.push(dir);
  ...
  registry.open({dispatchId:'0123456789abcdef',peer:'paired-credential-a',ownerSessionId:'manager',localSessionId:'local-worker'});
  registry.attachSession('0123456789abcdef','remote-worker');
  const update = {protocol:DISPATCH_PROTOCOL,dispatchId:'0123456789abcdef',sessionId:'remote-worker',seq:1,ts:1,final:true,kind:'worker-finished',entry:{sessionId:'remote-worker',label:'Worker',fullReply:'done'}} as const;
...
it('binds results to the recorded destination and actual worker, never a supplied parent', () => {
  ...
  expect(registry.accept('paired-credential-b',update)).toEqual({ok:false,reason:'wrong-peer'});
  expect(registry.accept('paired-credential-a',{...update,sessionId:'other'})).toEqual({ok:false,reason:'malformed'});
  expect(registry.accept('paired-credential-a',{...update,sessionId:'other',entry:{...update.entry,sessionId:'other'}})).toEqual({ok:false,reason:'session-mismatch'});
  expect(registry.accept('paired-credential-a',{...update,parentSessionId:'victim'})).toMatchObject({ok:true,parentSessionId:'manager'});
});
...
it('keeps uncertain admission open and transfers origin ownership on adoption', () => {
  ...
  expect(registry.accept('paired-credential-a',update)).toMatchObject({ok:true,parentSessionId:'successor'});
});
