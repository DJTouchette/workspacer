...
import type { createFleetWorkflowRuntime } from './fleetWorkflowCore';
import type { ClaudeSessionState } from './claudeSessionStore';
export function createFleetWorkflowService(runtime: ReturnType<typeof createFleetWorkflowRuntime>, getSnapshot: (id: string) => Pick<ClaudeSessionState, 'sessionId' | 'cwd' | 'isWakeTarget' | 'status' | 'hub' | 'label'> | undefined, templates: () => WorkflowTemplate[]) {
const { workflowBusy, ownerTask, workflowInstructions, workflowDispatchPlan, configuredWorkflowProjectKey } = runtime;
const fleetWorkflowStore = new FleetWorkflowStore(
...
function fleetWorkflowRequest(
  ...
  try {
    ...
    if (op === 'start') {
      const owner = callerSessionId ? getSnapshot(callerSessionId) : undefined;
      if (
        !owner?.isWakeTarget ||
        owner.hub ||
        owner.status === 'ended' ||
...
return { fleetWorkflowStore, fleetWorkflowRequest };
}
