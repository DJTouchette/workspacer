...
export function createFleetWorkflowService(
  runtime: ReturnType<typeof createFleetWorkflowRuntime>,
  getSnapshot: (
    id: string,
  ) =>
    | Pick<ClaudeSessionState, 'sessionId' | 'cwd' | 'isWakeTarget' | 'status' | 'hub' | 'label'>
    | undefined,
  ...
  function freeformTaskInstructions(
    task: import('../shared/dispatchHistory').DispatchTask,
  ): string {
