...
export async function nativeDesktopService(
  method: string,
  raw: unknown,
  context: HostContext,
): Promise<unknown> {
  ...
  switch (method) {
    ...
    case 'desktop.managerReplacement':
      ...
      return requestManagerReplacement(
        p.request as Parameters<typeof requestManagerReplacement>[0],
      );
    case 'desktop.agentRuntimeStatus':
      return readAgentRuntimeStatus();
    case 'desktop.providerReadiness':
      if (typeof p.provider !== 'string') throw new Error('Invalid provider');
      return p.check === true
        ? providerReadinessService.check(p.provider)
        : providerReadinessService.read(p.provider);
    case 'desktop.keepWarmHeartbeats': {
      const limit =
        typeof p.limit === 'number' ? Math.min(200, Math.max(1, Math.floor(p.limit))) : 20;
      const res = await fetch(`${CLAUDEMON_API_URL}/heartbeats?limit=${limit}`, {
    ...
    case 'desktop.managerRequestSend': {
      if (typeof p.sessionId !== 'string' || typeof p.requestId !== 'string' || !p.requestId)
      ...
      if (delivery) {
        try {
          ...
          /* Return the durable receipt; never replay through another path. */
        }
      }
      const receipt = service.request(managerReplacementState.wakeTarget(target), p.requestId);
      return {
        ok: receipt.delivery === 'accepted' || receipt.delivery === 'pending',
    ...
    default:
      return desktopHostCall(method, p, { ...context, daemonURL: CLAUDEMON_API_URL });
  }
}
