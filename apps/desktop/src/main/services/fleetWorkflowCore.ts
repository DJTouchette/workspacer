...
export function createFleetWorkflowRuntime(
  getSnapshot: (
    id: string,
  ) => Pick<ClaudeSessionState, 'sessionId' | 'isWakeTarget' | 'status' | 'hub'> | undefined,
) {
  ...
  function workflowDispatchPlan(task: DispatchTask): WorkflowDispatchPlan | undefined {
    ...
    if (
      taskDependencyState(task, task.dependsOn?.length ? dispatchHistoryStore.list() : []) !==
      'ready'
    )
      return;
    const pin = task.workflow;
  ...
  function workflowSpawn<T>(
    ...
    return async (raw) => {
      ...
      const task = ownerTask(
        p.taskId as string,
        p.dispatchOwnerSessionId as string,
        p.cwd as string,
      );
      if (
  ...
  return {
    ...
    workflowSpawn,
    pinnedWorkflowTemplate,
    workflowWakeInstructions,
