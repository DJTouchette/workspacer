...
export function createFleetWorkflowRuntime(getSnapshot: (id: string) => Pick<ClaudeSessionState, 'sessionId' | 'isWakeTarget' | 'status' | 'hub'> | undefined) {
const workflowBusy = new Set<string>();
/**
...
function ownerTask(
  ...
  const owner = caller ? getSnapshot(caller) : undefined;
...
function workflowInstructions(task: DispatchTask): string {
...
function workflowDispatchPlan(task: DispatchTask): WorkflowDispatchPlan | undefined {
...
function pinnedWorkflowTemplate(taskId: string, stepId: string): WorkflowTemplate {
...
function workflowWakeInstructions(sessionIds: string[]): string {
...
function workflowResultSchema(sessionId: string): Record<string, unknown> | undefined {
...
function configuredWorkflowProjectKey(cwd: string): string {
...
function missingWorkflowEvidence(task: DispatchTask, index: number): string | undefined {
  const pin = task.workflow!;
  const step = pin.definition.steps[index];
...
return { workflowBusy, ownerTask, workflowInstructions, workflowDispatchPlan, workflowSpawn, pinnedWorkflowTemplate, workflowWakeInstructions, workflowResultSchema, configuredWorkflowProjectKey, missingWorkflowEvidence };
}
