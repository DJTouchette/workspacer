...
vi.mock('./managerReplacementState', () => ({
  managerReplacementState: {
    holdMessage: vi.fn(() => false),
    noteInFlightMessage: vi.fn(),
    acknowledged: () => false,
  },
}));
vi.mock('./managerRequestService', async (original) => ({
  ...(await original<typeof import('./managerRequestService')>()),
  managerRequests: () => transport.service,
}));
...
it('uses real HTTP admission with unchanged text; delayed/lost/rejected acknowledgements retain logical identity', async () => {
  ...
  transport.service = new ManagerRequestService(
    history,
  ...
  const server = createServer(async (req, res) => {
    ...
    release = () => {
      ...
      res.end(JSON.stringify({ ok: status === 200, queued: true }));
    };
  });
  ...
  try {
    ...
    status = 500;
    const lost = claudemonSessionClient.message('manager', retry.text, undefined, {
      ...retry,
    ...
    await expect(
      claudemonSessionClient.messageDirect('manager', retry.text, {
        ...retry,
        requestId: b.requestId,
      }),
    ).rejects.toThrow(/must not replay/);
    expect(bodies).toHaveLength(3);

    vi.mocked(managerReplacementState.holdMessage).mockReturnValueOnce(true);
    const held = prepare();
    expect(
      await claudemonSessionClient.message('manager', held.text, undefined, held),
    ).toMatchObject({ mode: 'handoff-queued' });
    ...
    release = undefined;
    await transferred;
    expect(transport.service.request('successor', held.requestId)).toMatchObject({
