...
import { startHub, stopHub, isHubAdopted } from './services/hubDaemon';
        ...
        startHub()
          .then(() => {
            ...
            startMcpFacade()
              .then(() => {
                if (!isHubAdopted())
                  void enableManagerReplacementHost().catch((error) =>
                    console.warn('[manager-handoff] recovery needs attention', error),
                  );
              })
              .catch((err) => {
                notifySystem({
