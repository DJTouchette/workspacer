...
fn render_runs(f: &mut Frame, area: Rect, app: &App) {
    let t = &app.theme;
    let block = Block::default()
        .borders(Borders::ALL)
        .title(" runs ")
...
fn workflow_lines(t: &Theme, run: &crate::runs::WorkflowRun, width: u16) -> Vec<Line<'static>> {
    let mut out = Vec::new();
...
fn state_color_for(t: &Theme, state: crate::runs::RunState) -> ratatui::style::Color {
    match state {
