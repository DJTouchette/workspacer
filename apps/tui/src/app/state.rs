...
pub enum PickerKind {
    ...
    /// all providers POST the structural `/model` owner endpoint; claudemon
    /// alone constructs a validated Claude PTY command. `effort` rides along
    /// only for providers that genuinely apply it (for example codex).
    Model {
        provider: String,
