...
- Comparison uses unrounded used minus expected: inclusive ±2 percentage points is “on pace”; greater than +2 is “ahead”; below −2 is “under.” Ahead has warning colour and “spending faster than expected” accessible text. (SUPERSEDED on the usage-schedule branch: the band is unchanged, but the presentation of that state is now the label “above pace” in the error colour, on the word and the bar together — see `docs/reviews/overview-usage-schedule.md`.) Unknown, disabled, malformed, expired, and explicitly stale inputs show no pace verdict/tick. Zero consumes zero bar width.
