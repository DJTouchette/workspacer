...
> Current-state catalog of what the app does, as of 2026-06-13 (after the
> production-readiness pass). For the keep/drop triage and dead-code notes, see
...
A local-first cockpit for running **many long-lived Claude Code agents side by
side**. An Electron + React desktop app is the primary client; a Rust daemon
(`claudemon`) owns the sessions/PTYs and parses transcripts; a Go control-plane
...
| Spawn agent | Pick cwd / model / profile / skip-permissions; seed an initial prompt | 🔵 Working |
...
## 2. Claude agent interaction

| Feature | What it does | Maturity |
|---|---|---|
| Claude pane — terminal mode | Live PTY view of the Claude session | 🔵 Working |
| Claude pane — GUI mode | Rich conversation view: approve/deny, answer questions, inline diffs, work log | 🔵 Working |
| Inspector rail | Files changed / Workflows / Subagents / Usage for the active session | 🔵 Working |
| Composer | Send messages with file attach (drag / paste / picker); streaming + cancel | 🔵 Working |
| Approvals & questions | One-key approve/deny and AskUserQuestion answering, from the pane or sidebar | 🔵 Working |
| Subagent & workflow telemetry | Live phases, per-agent tokens/tools surfaced from the daemon's workflow watcher | 🔵 Working |
| Claude profiles | Named profiles (model, config dir, extra args) selectable at spawn | 🔵 Working |
...
| Settings | 8 sections, all persisted | 🔵 Working |
...
| Keybindings | Default & vim modes, leader chords, remap capture, shortcut overlay | 🔵 Working |
| View modes | tabs (horizontal strip) / spatial (pan-zoom canvas) / stacked (vertical feed) — without remounting live panes | 🔵 Working |
| Fleet Deck | Cross-agent "radar" of live agent cards, a global altitude orthogonal to view mode | 🔵 Working |
...
| Themes | ~18 theme variants via `--wks-*` CSS tokens | 🔵 Working |
...
| Remote web client (`/remote`) | Self-contained phone/PC client: agent list, chat, approvals, live xterm terminal mirror | 🔵 Working |
| Full web app (`/app/`) | Serves the real renderer bundle over the bus (when remote sharing + web build present) | 🔵 Working |
| Remote sharing | Opt-in (`WORKSPACER_REMOTE_SHARE`); binds off-loopback with a bearer token; QR share dialog | 🔵 Working |
...
| MCP tools (10) | list_agents, get_transcript, spawn_agent, create_terminal, send_message, approve, answer, signal, terminal_input, notify | 🟢 Solid |
...
| Command palette, vim nav | Ctrl-K fuzzy palette; vim-style keys | 🔵 Working |
