...
| Headless server (`workspacer serve`) | One command starts + supervises claudemon, the hub with a full-scope brain provider, and the loopback authenticated MCP facade used by spawned agents (restart backoff, parentwatch); prints the /m + /remote (plus bus + claudemon) URLs and pairing token (`--json` for machines; `/app` is auto-served when the bundled `web/` build is present but gets no banner line); loopback by default, `--host` is the remote opt-in; `workspacer status` probes the control plane | 🔵 Working |
...
| Known limits | No UI, by design (no routing write RPC is what makes a ceiling enforceable). `forecast_weights` produce weighted units, not a share of an allowance, so a work-only forecast leaves demand UNKNOWN for the mode rules; `difficulty`/`risk`/`decisionDensity` are accepted and not yet acted on; `roles.supervisor` is not consulted (the manager's own model comes from `agents.managerProvider`/`managerModels`, and Settings → Fleet Manager says so). Nothing calls `routing.select` on a spawn: the matrix answers a dispatch only when the caller asks it to, so a caller that names no role gets no decision and no freshness claim | 🟡 Partial |
...
| Git review pane | ❌ Removed. It spoke to claudemon `/git/*`, and those routes moved to the desktop host in `37320188`; a live daemon answered 404. The pane, its `R` / `<leader>g` / `:review` bindings and its client methods were deleted rather than rewired — git review is desktop-only. | ⚪ Not built |
| Tests | Behavioral unit coverage for navigation, approval/question handling, direct mode, bus mode, and reconnect behavior | 🟢 Solid |
