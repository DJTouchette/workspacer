...
Admission refuses an ineligible step, missing binding, foreign manager/project, contract/message override or session resume/retry. A per-task admission lock spans asynchronous allocation. Every configured review uses a fresh worker; an implementer's session cannot be reused. Unrelated standalone launches retain the previous metadata behavior and historical tasks are never retroactively bound.
