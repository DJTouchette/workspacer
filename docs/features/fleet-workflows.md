...
First-task guidance links to both policy editors. The workflow editor’s Model routing link opens Settings → Routing; the return link opens the existing Fleet Manager section (`supervisor`) containing Workflows. Links in an active editor are disabled until the draft is closed/applied.
...
The manager calls `select_model` with the step role, then `spawn_agent` with its routing answer and exact workflow metadata. Research/review/validation remain read-only by contract and instruction; implementation/repair/landing require isolated worktree allocation. Definitions cannot choose providers, models, capabilities, permissions or worktree settings. Model ceilings, provider-native permission modes and project delivery policy remain authoritative; supported agents share the ambient Workspacer tool surface.
