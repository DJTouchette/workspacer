...
| `/plugins/remove`, `/inspect`, `/updates`, `/setEnabled`, `/settings`, `/plugins/tokens`, `/plugins/pane-token[/revoke]` | **Yes** | Mutating / sensitive plugin routes. |
| `/plugins/install`, `/plugins/examples/install`, `/plugins/reload` | **Yes — and the HOST token specifically** | The install family runs code on the hub's own machine (the manifest's install argv; reload starts a sidecar from a caller-named directory). The bus token alone is not the gate here, because an operator-tier *scoped* token passes `Authorized` — and that is exactly the credential a remote worker node carries, since attaching as a capability provider requires trust. So these three ask for `Server.HostAuthorized`: the host token only, everything else 403. Interim measure; the durable fix is a provider tier so `conn.mayProvide` stops forcing host authority. |
