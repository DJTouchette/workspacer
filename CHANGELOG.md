...
### Added
- Ordinary agents can spawn direct children and receive their progress, completion,
  and blocker updates. Project skills guide child spawning and atomic project-brief
  maintenance while Fleet Managers retain their task and workflow lifecycle.
- The chat slash picker now offers `spawn-agent` and `project-brief` for ordinary
  Codex, Claude Code, GitHub Copilot, and OpenCode sessions, including remote chats.
  Selecting a skill inserts its instructions into an editable draft.

### Changed
- Redesigned New Agent with a provider-logo sidebar, a compact form for checkout,
  name, model, effort, permissions, mode, and context, and a launch-summary footer.
  The layout adapts to narrow screens and keeps extra options under Advanced.
- Simplified the launch sidebar to a compact runtime indicator with expandable
  diagnostics and a refresh action for runtime, provider, and folder checks.
- Authenticated agent sessions receive Workspacer tools and enabled plugin tools
  without per-session grant selection. Provider-native permission modes remain
  configurable; lifecycle-bound identities preserve parent routing and provenance.

### Fixed
- Kept ordinary-agent collaboration skills out of Fleet Manager discovery.
- Hardened facade readiness and identity cleanup during agent launch and shutdown,
  and aligned collaboration behavior and tests across desktop and headless hosts.
...
- Removed the Intent workspace preview and its board, tracker synchronization, and
  ...
  requests, tracked tasks, workflows, and task references remain available.
...
- **Overview's weekly usage pacing can follow a five-day working week.**
  ...
  on a 7-day card climbs Monday to Friday and stays exactly where it is across
  Saturday and Sunday, in the hub machine's own timezone — so a week's allowance
  spent over five working days reads as on plan rather than 40% over it, and
  Sunday afternoon no longer suggests you are behind. Weekend work still counts:
  only the expectation pauses, never the observed figure. *Every day* is the
  calendar shape the cards shipped with. Leaving the setting untouched changes
  nothing at all, including for anyone who has hand-set a curve in the hub's
  `routing.yaml`, which keeps answering until a choice is made here. A window
  whose elapsed part is entirely weekend shows no pace verdict rather than a
  fabricated one, because there is no weekday progress yet to compare against.
  The 5-hour and monthly cards are untouched by the setting either way, and
  neither is any routing decision: the schedule is read only when the usage
  report is projected, never when a model is chosen. The choice is stored by
  the hub, so it survives a restart and applies to the desktop, the web client
  and the phone alike; changing it re-reads the visible cards immediately. Only
  a host-authority client may change it — a phone or a scoped pairing token can
...
- **The hub no longer takes five seconds to start, so "the control plane is
  ...
  catalog check now fires five seconds after the routing service starts,
...
- **Effort stepping: a routing mode can now trim thinking time instead of
  ...
  reasoning ladder, applied to the row the answer lands on. Conserve
  ...
  where it was, for the band that needs it. That pace-armed trim is
  per-provider by its own rationale: when a fallover lands the answer on a
  different provider, the band is re-read for the provider that will run the
  work, and the trim applies only if that provider is itself running ahead.
  A conserve or spend_down step is not re-derived, because the mode owns the
  whole decision. Under conserve both levers can fire on one answer: a
  `judge` on `codex_only` drops from `frontier_plus` to `deep_reviewer` and
  from `high` to `medium` at once.
  ...
  a conserving fleet from trimming its own reviewer. A floor written ABOVE
  the row's own `effort:` is a contradiction the loader reports, and it
...
- **Live provider availability is a fallover trigger.** Every reason the
  ...
  row switched off, a load-time issue, a health reading. One live fact was
  missing: a provider whose CLI runs and reports that it can launch no model
  at all, however healthy its allowance looks. The hub's existing
  per-provider model-catalog probe now also answers that question, and the
  map is injected into the decision rather than fetched inside it, so the
  policy layer stays pure.

  A provider whose CLI ran and listed no launchable model is routed around,
  with the reason naming it. Everything else is unknown and routes exactly as
  it did before, which is what keeps a hub that cannot reach its daemon for
  thirty seconds from declaring every provider dead.
  ...
  `alternatives:`, and the provider a mode shift lands on are not checked
  against it.

  A verdict that found models is cached for the catalog TTL. An `unavailable`
  verdict and an unreachable daemon are both re-probed on the next background
  refresh, because a provider somebody has just installed or logged into is
  exactly the one whose verdict has gone stale.
