...
- `services/hub/internal/bus/rpc.go` + `services/hub/cmd/hub/main.go` — `srv.SetFederation(fed)` wires qualified routing: on `call`, a `hub:<peer>/` prefix is stripped for the scoped-token tier check (bare method against exact allowlists). Plugin identities are local provenance and cannot federate calls. Paths name the PEER filesystem and are handled there under the authenticated caller's ambient authority; capspec still validates parameter shape and semantic object boundaries. `federation.peers` is a hub-local view method.
- `apps/desktop/src/main/services/federationBridge.ts` — ingests hub-stamped snapshots, seeds each peer on connect, marks tombstones, and routes actions to the owning hub. `snapshotIsLocalLiveSession` excludes hub-stamped rows from local-directory context: a peer cwd is not a path on this machine. That is provenance, not a filesystem grant. Keep-warm and seen-model recording likewise stay on the owning hub.
...
- **Remote sessions must not be mistaken for local sessions**:
  `snapshotIsLocalLiveSession` excludes peer rows from local-directory context;
  keep-warm, seen-models and cwd-bound panes stay on the owning hub. When adding
  a feature that reads cwd, opens files or runs git, route by `snapshot.hub`.
