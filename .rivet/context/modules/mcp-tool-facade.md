...
- **no credential** → operator, ONLY when no static token is set (the loopback-open back-compat default);
...
## Gotchas
- **The untokened-loopback = operator default is back-compat, not the norm anymore.** `mcpFacadeDaemon.ts` still never sets `WKS_MCP_TOKEN`, so a bare local client (nothing presenting a token) gets operator on 127.0.0.1 — but every *spawned session* since the tier work carries a scoped per-session token and gets its tier. The untokened operator default also gets **no plugin tools** (opt-in is per token record). If the facade ever binds non-loopback, `mcpFacadeDaemon.ts` must generate and pass a token, not just change the addr (bind policy fails closed).
...
- **The facade is fleet-wide (since 2026-08-16)**: `list_agents`/`list_snapshots` merge connected federation peers (rows tagged `hub`; per-peer failures cost only their rows), and the per-session tools + `spawn_agent` take an optional `hub` param routed as `hub:<peer>/<method>` (`services/hub/cmd/mcp/federated.go`; the param is stripped before forwarding). The `-untokened operator|view|deny` dial governs credential-less access (env `WKS_MCP_UNTOKENED`; `deny` alone satisfies the non-loopback bind policy). See `hub-federation`.
