...
`workspacer` (package `main` at `services/hub/cmd/workspacer/`) is the standalone launcher binary for headless server mode. `workspacer serve` resolves, launches, and supervises claudemon (the Rust session daemon), the hub (the Go event bus, started with `--brain-scope full`), and the Workspacer MCP facade. It waits for the facade to identify itself, connect to the expected hub, and load its initial plugin catalog before starting the brain with that verified endpoint. The launcher wires their ports and shared host authentication together and prints the URLs + pairing token a remote client (`/remote`, `/m`, the TUI, MCP) needs to connect. This binary holds launch/supervise/report rather than product logic. It also exposes `workspacer status`, `workspacer token` for separately tiered remote-client credentials, and `workspacer install-cli`. It is built by `make build-cli`, distinct from the desktop app's daemon-spawning path.
