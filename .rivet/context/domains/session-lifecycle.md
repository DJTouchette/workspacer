...
- **New-capability checklist:** keep desktop/headless registration, capspec
  parameter/composition classification, remote-token allowlists and MCP tool
  metadata in sync. There is no renderer-side plugin permission registry in the
  current ambient-plugin model.
