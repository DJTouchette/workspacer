...
pub fn ensure(claudemon_url: &str, enabled: bool) -> Daemons {
    ...
    match claudemon_bin() {
        ...
        None => eprintln!(
            "[wks-tui] claudemon not found — build it (cargo build --release in claudemon/) \
...
// this crate. `CARGO_MANIFEST_DIR` is `<repo>/wks-tui`, so its parent is the
// repo root — the common case when running via cargo or the in-tree binary.
...
fn claudemon_bin() -> Option<PathBuf> {
    ...
    let target = repo_root().join("claudemon").join("target");
