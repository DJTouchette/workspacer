...
//!
//! Pi also has NO MCP support in core (its README: "No MCP." — extensions are
//! the extension point), so anything other providers get via MCP config must
//! ride pi's extension API instead: AskUserQuestion is a generated `-e`
//! extension (see [`ASK_EXTENSION_TEMPLATE`]) and the supervisor facade is
//! currently unavailable (see the warning in [`run_session`]).
//!
...
pub fn spawn_session(
    ...
    tokio::spawn(async move {
        ...
        // extension (pi has no MCP; see ASK_EXTENSION_TEMPLATE for the why).
        let ask_ext = write_ask_extension(&session_id);
...
/// The generated AskUserQuestion extension, parameterized only by the
/// per-session ask URL (spliced in as a JSON string literal, so any URL stays a
/// valid TS string).
///
/// Why an extension and not MCP config: pi core has NO MCP client at all (its
/// README: "No MCP. Build … an extension that adds MCP support."), so the
/// daemon's `/mcp/ask/:session_id` endpoint can't be registered the way codex
/// and opencode do it. Pi's native seam for new tools is the extension API —
/// and because the ask endpoint is streamable HTTP in its simplest legal form
/// (one JSON-RPC POST in, one JSON response out, no SSE), the extension needs
/// no MCP client either: one `node:http` request per question suffices.
/// `node:http` rather than `fetch` on purpose — the endpoint sends *nothing*
/// (not even headers) until the user answers, legitimately hours later, and
/// node's fetch (undici) gives up on headerless responses after ~5 minutes.
const ASK_EXTENSION_TEMPLATE: &str = r##"// Generated by workspacer (claudemon) for one pi session - do not edit.
// Registers an AskUserQuestion tool that forwards to the daemon, which parks
// a structured question card in the GUI and responds once the user answers.
import { request } from "node:http";
import type { ExtensionAPI } from "@earendil-works/pi-coding-agent";
import { Type } from "typebox";

const ASK_URL = __ASK_URL__;

function callAsk(args: unknown, signal?: AbortSignal): Promise<string> {
  return new Promise((resolve, reject) => {
      ...
      (res) => {
        ...
        res.on("end", () => {
          try {
            ...
            if (rpc.error) throw new Error(rpc.error.message || "AskUserQuestion failed");
            const content = Array.isArray(rpc.result?.content) ? rpc.result.content : [];
            const text = content
              .filter((c: { type?: string }) => c && c.type === "text")
              .map((c: { text?: string }) => c.text || "")
              .join("\n");
            // Tool-level failure (answer timeout, session closed) - throw so
            // pi reports isError to the LLM instead of a fake answer.
            if (rpc.result?.isError) throw new Error(text || "AskUserQuestion failed");
    ...
    req.end(
      JSON.stringify({
        ...
        id: 1,
        method: "tools/call",
        params: { name: "AskUserQuestion", arguments: args },
      }),
    );
  });
}
...
export default function (pi: ExtensionAPI) {
  // Name, schema, and description mirror Claude Code's native AskUserQuestion
  // (and the daemon endpoint's MCP tool), so questions parse straight into the
  // GUI's picker and prompts written for Claude keep working under pi.
  pi.registerTool({
        ...
        Type.Object({
          ...
          options: Type.Array(
            Type.Object({
              ...
              description: Type.Optional(Type.String({ description: "What picking this option means" })),
            }),
          ),
        }),
      ),
    ...
    async execute(_toolCallId: string, params: unknown, signal?: AbortSignal) {
      const text = await callAsk(params, signal);
...
/// Render the ask extension for one session. The URL is embedded through a
/// JSON string encoder so a hostile/odd URL can't escape the TS string literal.
fn ask_extension_source(ask_url: &str) -> String {
    let literal = Value::String(ask_url.to_string()).to_string();
    ASK_EXTENSION_TEMPLATE.replace("__ASK_URL__", &literal)
}

/// Materialize the per-session ask extension for `-e <file>`. `-e` loads
/// pre-trust and works in both RPC and TUI modes, and — unlike a config file —
/// keeps the per-session URL out of shared/project pi config where it would go
/// stale or leak between sessions. Lives in the OS temp dir keyed by session
/// id; the spawn wrapper removes it when the session ends (a leaked file is
/// inert). Returns None when the HTTP API isn't up (unit tests) or the write
/// fails — the session still works, questions just stay prose.
fn write_ask_extension(session_id: &str) -> Option<std::path::PathBuf> {
    let api_base = crate::daemon::API_BASE.get()?;
    let path = std::env::temp_dir().join(format!("workspacer-pi-ask-{session_id}.ts"));
    let source = ask_extension_source(&format!("{api_base}/mcp/ask/{session_id}"));
    match std::fs::write(&path, source) {
        Ok(()) => Some(path),
        Err(err) => {
            tracing::warn!(
                ?err,
                "writing the pi ask extension failed; questions stay prose"
            );
            None
        }
    }
...
/// Argv for the hybrid's native TUI: pinned to our canonical session id, plus
/// the AskUserQuestion extension when available (`-e` loads pre-trust in every
/// mode and keeps the per-session URL out of shared config).
fn tui_argv(
    bin: &str,
    session_id: &str,
    model: Option<&str>,
    ask_ext: Option<&std::path::Path>,
) -> Vec<String> {
    let mut argv = vec![
        bin.to_string(),
...
/// in the Term. No structural interrupt either — pi's `abort` only exists on
/// the `--mode rpc` stdio channel, which the TUI doesn't have — but the PTY
/// Ctrl-C pump already covers interrupts here (the same capability cliff as
/// codex's rollout fallback).
...
async fn run_session(
    ...
    // Supervisor facade: pi core ships NO MCP client at all (its README: "No
    // MCP. Build … an extension that adds MCP support."), so there is no
    // config seam to hand `facade.mcp_url` through — the `.mcp.json` write
    // this used to do was a silent no-op pi never read. Until an MCP-bridge
    // extension exists (it would need a real streamable-HTTP client for the
    // facade, unlike the one-POST ask endpoint), a pi supervisor runs on role
    // instructions alone; warn so the missing control plane is visible.
    if facade.mcp_url.is_some() {
        tracing::warn!(
            session = %session_id,
            "pi has no MCP support; workspacer facade tools are unavailable to this supervisor"
        );
    }
    ...
    // AskUserQuestion rides a generated extension, not MCP config — `-e` loads
    // it pre-trust and in RPC mode too (see ASK_EXTENSION_TEMPLATE).
    if let Some(ext) = ask_ext {
        cmd.arg("-e").arg(ext);
    }
    let mut child = cmd
    ...
    // Structural interrupt (POST /sessions/:id/signal SIGINT): pi's RPC has a
    // first-class `abort` that stops the current agent run and keeps the
    // session alive — same semantics as codex's `turn/interrupt` — and it is
    // the ONLY interrupt this path has at all: `--mode rpc` runs without a PTY,
    // so with no channel registered a SIGINT would 404 and a runaway turn could
    // only be killed whole. Pi reports the outcome on the stream itself (an
    // `error` with reason "aborted", then `agent_end` flips us back to Input),
    // so nothing to update locally when one fires.
    ...
    loop {
        tokio::select! {
            ...
            intr = irx.recv() => match intr {
                Some(()) => {
                    // Idempotent and safe when idle: pi acks with a plain
                    // `response` (dropped by handle_message) either way.
                    let _ = write_msg(&mut stdin, &json!({ "type": "abort" })).await;
                }
                None => break,
            },
...
mod tests {
    ...
    // ── AskUserQuestion extension (pi's stand-in for MCP registration) ──

    #[test]
    fn ask_extension_embeds_the_session_url_and_tool() {
        let src = ask_extension_source("http://127.0.0.1:7787/mcp/ask/abc-123");
        assert!(src.contains(r#"const ASK_URL = "http://127.0.0.1:7787/mcp/ask/abc-123";"#));
        assert!(!src.contains("__ASK_URL__"));
        // The tool name must match the daemon endpoint's (and Claude's native)
        // AskUserQuestion, so questions parse straight into the GUI picker.
        assert!(src.contains(r#"name: "AskUserQuestion""#));
        // A pi extension is a default-exported factory.
        assert!(src.contains("export default function"));
    }

    #[test]
    fn ask_extension_url_cannot_escape_the_string_literal() {
        // A hostile/odd URL is JSON-escaped into the TS literal, not spliced raw.
        let src = ask_extension_source(r#"http://x/"; process.exit(1); ""#);
        assert!(src.contains(r#"const ASK_URL = "http://x/\"; process.exit(1); \"";"#));
    }

    #[test]
    fn tui_argv_pins_session_and_loads_the_ask_extension() {
        let argv = tui_argv(
            "pi",
            "sid-1",
            Some("anthropic/claude-sonnet-4-5"),
            Some(std::path::Path::new("/tmp/ask.ts")),
        );
        assert_eq!(
            argv,
            vec![
        ...
        // Without a model / extension the flags are simply absent.
        assert_eq!(
