...
const tmp = fs.mkdtempSync(
  path.join(os.tmpdir(), "wks-routing-limit-harness-"),
);
...
function check(name, cond, extra = "") {
  ...
  const ok = !!cond;
  console.log(
    `${ok ? "  [PASS]" : "  [FAIL]"} ${name}${!ok && extra ? ` - ${extra}` : ""}`,
  );
  if (!ok) failures++;
}
...
async function startFake(port) {
  ...
  child.stderr.on(
    "data",
    (d) => process.env.HARNESS_VERBOSE && process.stderr.write(d),
  );
  ...
  return new Promise((resolve, reject) => {
    ...
    child.on("exit", (code, signal) => {
      if (!settled) {
        ...
        reject(
          new Error(
            `fake claudemon exited early: code=${code} signal=${signal}`,
          ),
        );
      }
    });
...
function startHub(hubPort, fakeURL) {
  ...
  hub = spawn(
    ...
    [
      ...
      "",
      "-layout-file",
      path.join(tmp, "layout.json"),
      "-push-dir",
      path.join(tmp, "push"),
      "-peers-file",
      path.join(tmp, "peers.json"),
  ...
  hub.stdout.on(
    "data",
    (d) => process.env.HARNESS_VERBOSE && process.stdout.write(d),
  );
}
...
function connect(hubPort, name, token = TOKEN) {
  return new Promise((resolve, reject) => {
    ...
    ws.onerror = (e) =>
      reject(new Error(`ws ${name}: ${e.message ?? "error"}`));
    ws.onmessage = (m) => {
...
const fakeRequestCount = async (fakeURL) =>
  (await fakeJSON(fakeURL, "/usage/requests")).count ?? 0;
...
const modelProbeCount = async (fakeURL, provider) =>
  ((await fakeJSON(fakeURL, "/providers/probes")).probes ?? []).filter(
...
function allStrings(v, out = []) {
  if (typeof v === "string") out.push(v);
    ...
    Object.values(v).forEach((x) => allStrings(x, out));
  return out;
}
...
function assertUnknownNamed(name, result) {
  check(
    `${name}: limit state is explicitly UNKNOWN`,
    hasExactString(result, "unknown") || serialized(result).includes("unknown"),
    JSON.stringify(result),
  );
  check(
    `${name}: no GREEN/healthy default leaked`,
    !hasExactString(result, "green") && !hasExactString(result, "healthy"),
    JSON.stringify(result),
  );
}
...
function assertNoStaleModes(name, result) {
  check(
    `${name}: stale/non-current window did not choose CONSERVE`,
    !hasExactString(result, "conserve"),
    JSON.stringify(result),
  );
  check(
    `${name}: stale/non-current window did not choose SPEND_DOWN`,
    !hasExactString(result, "spend_down"),
    JSON.stringify(result),
  );
}
...
async function runPreferencesAssertions(caller, hubPort, fakeURL) {
  ...
  check(
    "preferences expose compiled defaults and no security mappings",
    initial.schemaVersion === 1 &&
      initial.defaults.activeProfile === "mixed" &&
      !JSON.stringify(initial).includes("maxToolScope"),
  );
  const request = {
    baseRevision: initial.revision,
    patch: { roles: { mechanical: "balanced" }, modes: { global: "normal" } },
  };
  const validation = await caller.call("routing.preferences.validate", request);
  check(
    "typed draft validates without writing a sidecar",
    validation.status === "valid" &&
      !fs.existsSync(path.join(tmp, "routing-preferences.json")),
  );
  ...
  const selected = await caller.call("routing.select", {
    role: "mechanical",
    cwd: tmp,
  });
  check(
    "production routing.select consumes the saved role",
    selected.baseCapability === "balanced",
  );
  ...
  const selectedEvent = await waitFor(
    async () =>
      caller.events.find(
        (e) =>
          e?.type === "routing.decision" &&
          e?.data?.decisionId === selected.decisionId,
      ),
    5000,
  );
  check(
    "the preceding selection event is observed before preview",
    !!selectedEvent,
  );
  const auditBefore = fs.readFileSync(decisionLog, "utf8");
  const eventBefore = caller.events.length;
  const preview = await caller.call("routing.preview", {
    role: "mechanical",
    cwd: tmp,
  });
  check(
    "preview matches production choice without a decision id",
    preview.model === selected.model &&
      preview.effort === (selected.effort || "") &&
      !preview.decisionId,
  );
  await sleep(30);
  check(
    "preview emits no decision row or event",
    auditBefore === fs.readFileSync(decisionLog, "utf8") &&
      caller.events.length === eventBefore,
  );
  ...
  const operator = await connectWithRetry(
    hubPort,
  ...
  let refused = false;
  try {
    await operator.call("routing.preferences.save", {
      ...request,
      baseRevision: applied.view.revision,
    });
  } catch {
    refused = true;
  }
  ...
  const reset = await caller.call("routing.preferences.reset", {
    baseRevision: applied.view.revision,
  });
  check(
    "reset reveals inherited policy",
    reset.status === "applied" &&
      reset.view.effective.roles.mechanical ===
        initial.inherited.roles.mechanical &&
      reset.view.managedFields.length === 0,
  );
  check(
    "save and reset preserve trusted routing YAML byte-for-byte",
    hostBytes.equals(fs.readFileSync(path.join(tmp, "routing.yaml"))),
  );
}
...
async function maybeCallRouting(caller, scenario, request) {
  try {
    ...
    if (/no provider for routing\.select\b/.test(msg))
      return { ok: false, missingRouting: true, error: msg };
...
async function runRoutingCases(caller, fakeURL) {
  ...
  const cases = [
    {
      ...
      checks(result) {
        ...
        check(
          "stale-codex: response states the stale Codex bucket as UNKNOWN",
          serialized(result).includes("codex") &&
            serialized(result).includes("unknown"),
          JSON.stringify(result),
        );
      },
    },
    {
      ...
      checks(result) {
        ...
        check(
          "reset-now: boundary reset is UNKNOWN, not a current percentage",
          serialized(result).includes("unknown"),
          JSON.stringify(result),
        );
      },
    },
    {
      ...
      checks(result) {
        ...
        check(
          "copilot-403: response mentions the requested provider",
          serialized(result).includes("copilot"),
          JSON.stringify(result),
        );
      },
    },
    {
      ...
      checks(result) {
        ...
        check(
          "absent-opencode-pi: response mentions the absent requested provider",
          serialized(result).includes("opencode"),
          JSON.stringify(result),
        );
      },
    },
  ...
  for (const c of cases) {
    ...
    if (routingKnownMissing) {
      pending(
        `${c.scenario}: routing decision assertions`,
        "routing.select is not registered on this branch",
      );
      continue;
    }
    const call = await maybeCallRouting(
      caller,
      c.scenario,
      routingRequest(c.scenario, c.provider),
    );
    ...
    if (!call.ok) {
      check(
        `${c.scenario}: routing.select returned a result`,
        false,
        call.error,
      );
      continue;
    }
    const sawUsage = await waitFor(
      async () => (await fakeRequestCount(fakeURL)) > before,
      5000,
    );
    check(
      `${c.scenario}: hub read fake claudemon /usage/report`,
      !!sawUsage,
      "routing.select returned without touching the fake usage report",
    );
...
async function runPacingAssertions(caller, fakeURL) {
  ...
  const claudeWindows =
    provider(report, "claude")?.accounts?.[0]?.windows ?? {};
  check(
    "claude 5h window carries its 300-minute length",
    claudeWindows.five_hour?.window_minutes === 300,
    JSON.stringify(claudeWindows.five_hour),
  );
  check(
    "claude 7d window carries its 10080-minute length",
    claudeWindows.seven_day?.window_minutes === 10080,
    JSON.stringify(claudeWindows.seven_day),
  );
  check(
    "claude monthly window carries NO length (a calendar month is not fixed)",
    claudeWindows.monthly?.window_minutes == null,
    JSON.stringify(claudeWindows.monthly),
  );
  ...
  check(
    "a current claude window is PACED at all (the length arrived end to end)",
    okPace?.known === true,
    JSON.stringify(okPace),
  );
  check(
    "an on-schedule provider is on_track",
    okPace?.state === "on_track",
    JSON.stringify(okPace),
  );
  check(
    "an on-schedule provider is not conserved by pace",
    healthy.result?.mode !== "conserve",
    JSON.stringify(healthy.result?.reason),
  );
  ...
  check(
    "an over-curve window is OVERSPENDING",
    pace?.state === "overspending",
    JSON.stringify(pace),
  );
  check(
    "the binding window is named",
    pace?.window === "five_hour",
    JSON.stringify(pace),
  );
  check(
    "the ratio is above the shipped 1.25 band",
    typeof pace?.ratio === "number" && pace.ratio > 1.25,
    JSON.stringify(pace),
  );
  check(
    "health alone would NOT have conserved this (it is yellow, not red)",
    over.result?.capacity?.health === "yellow",
    JSON.stringify(over.result?.capacity?.health),
  );
  check(
    "the decision is CONSERVE",
    over.result?.mode === "conserve",
    JSON.stringify(over.result?.reason),
  );
  check(
    "the conserve is EXPLAINED in the answer, not merely asserted",
    (over.result?.reason ?? []).some((r) =>
      r.includes("faster than it refills"),
    ),
    JSON.stringify(over.result?.reason),
  );
  check(
    "every window's pace is reported, not only the binding one",
    Array.isArray(over.result?.capacity?.paceWindows) &&
      over.result.capacity.paceWindows.length >= 2,
    JSON.stringify(over.result?.capacity?.paceWindows),
  );
  ...
  const ev = await waitFor(
    async () =>
      over.events
        .concat(caller.events.slice(-20))
        .find(
          (e) =>
            e?.type === "routing.decision" &&
            e?.data?.decisionId === over.result?.decisionId,
        ),
    5000,
  );
  check(
    "the routing.decision event carries the pace state",
    ev?.data?.pace === "overspending",
    JSON.stringify(ev?.data),
  );
  check(
    "the routing.decision event carries the ratio and the window",
    ev?.data?.paceRatio > 1.25 && ev?.data?.paceWindow === "five_hour",
    JSON.stringify(ev?.data),
  );
  ...
  check(
    "a week running ahead of the curve is AHEAD, not overspending",
    blockPace?.state === "ahead",
    JSON.stringify(blockPace),
  );
  check(
    "the spend-down is blocked",
    blocked.result?.mode !== "spend_down",
    JSON.stringify(blocked.result?.reason),
  );
  check(
    "blocking a spend-down is not conserving",
    blocked.result?.mode === "normal",
    JSON.stringify(blocked.result?.reason),
  );
  check(
    "the block is explained",
    (blocked.result?.reason ?? []).some((r) =>
      r.includes("not going to expire unused"),
    ),
    JSON.stringify(blocked.result?.reason),
  );
  ...
  check(
    "a provider that publishes no window is pace-UNKNOWN",
    dark?.capacity?.pace?.known !== true,
    JSON.stringify(dark?.capacity?.pace),
  );
  check(
    "and it is still explained rather than silently dropped",
    typeof dark?.capacity?.pace?.because === "string" &&
      dark.capacity.pace.because.length > 0,
    JSON.stringify(dark?.capacity?.pace),
  );
  check(
    "an unpaceable provider is not conserved by pace",
    !(dark?.reason ?? []).some((r) => r.includes("faster than it refills")),
    JSON.stringify(dark?.reason),
  );
}
...
function seedFixtures() {
  ...
  fs.writeFileSync(
    path.join(tmp, "routing.yaml.seeded"),
    '{"seededVersion":1}\n',
  );
}
...
async function runDecisionRecordAssertions(caller, fakeURL) {
  ...
  check(
    "routing.select stamps a decisionId",
    typeof decisionId === "string" && decisionId.startsWith("rd_"),
    JSON.stringify(result),
  );
  ...
  check(
    "a routing.decision event is published for the answer",
    !!ev,
    JSON.stringify(caller.events.slice(before)),
  );
  if (ev) {
    check(
      "the event carries the model and mode the answer chose",
      ev.data.model === result.model && ev.data.mode === result.mode,
      JSON.stringify(ev.data),
    );
    check(
      "the event does NOT carry the caller cwd (open-by-decision topic)",
      ev.data.cwd === undefined,
      JSON.stringify(ev.data),
    );
    check(
      "the event names the ticket it was asked about",
      ev.data.ticketId === "HARNESS-REC-1",
      JSON.stringify(ev.data),
    );
  }
  ...
  const rows = await waitFor(async () => {
    ...
    return all.some((r) => r.kind === "decision" && r.decisionId === decisionId)
      ? all
      : null;
  }, 5000);
  check(
    "the decision is appended to routing-decisions.jsonl",
    !!rows,
    `no decision row for ${decisionId} in ${decisionLog}`,
  );
  if (rows) {
    const row = rows.find(
      (r) => r.kind === "decision" && r.decisionId === decisionId,
    );
    check(
      "the logged decision carries the model it answered",
      row?.decision?.model === result.model,
      JSON.stringify(row),
    );
    check(
      "the logged decision is timestamped",
...
async function runCeilingAwareSelectAssertions(caller) {
  ...
  check(
    "a select inside a capped directory still answers something spawnable",
    capped?.eligible === true,
    JSON.stringify(capped),
  );
  check(
    "the answer is capped to the directory ceiling",
    capped?.capability === "balanced",
    JSON.stringify(capped),
  );
  check(
    "the answer still records what the ROLE asked for",
    capped?.baseCapability === "frontier",
    JSON.stringify(capped),
  );
  check(
    "the decision reports the ceiling it was resolved under",
    capped?.ceiling?.capabilityRefused === true,
    JSON.stringify(capped?.ceiling),
  );
  check(
    "the cap is EXPLAINED in the answer, not only in a log",
    (capped?.reason ?? []).some((r) => r.includes("ceilings.")),
    JSON.stringify(capped?.reason),
  );
  ...
  check(
    "the advised tuple is complete enough to dispatch",
    !!provider && !!model,
    JSON.stringify(capped),
  );
  ...
  // everywhere.
  check(
    "the same role outside the capped tree is not capped at all",
    !free?.ceiling?.capabilityRefused,
    JSON.stringify(free?.ceiling),
  );
  check(
    "and it keeps a capability above the capped tree's ceiling",
    free?.capability !== "balanced",
    JSON.stringify(free),
  );
  ...
  check(
    "a mode-shift promotion is itself subject to the ceiling",
    capped?.capability === "balanced" &&
      capped?.reason?.some((r) => r.includes("mode_shifts")) !== undefined,
    JSON.stringify(capped?.reason),
  );
  ...
  check(
    "a SYMLINK to the capped directory is capped by routing.select too",
    viaLink?.capability === "balanced",
...
async function runSpawnBindingAssertions(hubPort, decisionId) {
  ...
  check(
    "the spawn reached the provider at all",
    !!over?.sessionId,
    JSON.stringify(over),
  );
  check(
    "capability was CLAMPED to the directory ceiling",
    got.capability === "balanced",
    JSON.stringify(got),
  );
  check(
    "the model the refused capability chose did not survive",
    got.model !== "fable",
    JSON.stringify(got),
  );
  ...
  check(
    "the clamp left no hole for a provider default to fill",
    typeof got.model === "string" && got.model.length > 0,
    JSON.stringify(got),
  );
  check(
    "the replacement model is what the PERMITTED capability resolves to",
    got.model === "sonnet",
    JSON.stringify(got),
  );
  check(
    "the replacement carries its own effort",
    got.effort === "high",
    JSON.stringify(got),
  );
  check(
    "the clamp did not swap the harness the spawn was for",
    got.provider === "claude",
    JSON.stringify(got),
  );
  check(
    "legacy toolScope is preserved but cannot narrow ambient tools",
    got.toolScope === "operator",
    JSON.stringify(got),
  );
  check(
    "only active routing fields are named in escalationScrubbed",
    Array.isArray(got.escalationScrubbed) &&
      ["capability", "model", "effort"].every((f) =>
        got.escalationScrubbed.includes(f),
      ) &&
      !got.escalationScrubbed.includes("toolScope"),
    JSON.stringify(got),
  );
  check(
    "the recorded routing metadata rode through untouched",
    got.role === "judge" && got.decisionId === decisionId,
    JSON.stringify(got),
  );

  // 2. UNDER the ceiling, same directory: nothing is taken.
  await operator.call("agents.spawn", {
    cwd: CEILED_DIR,
    capability: "cheap",
    model: "gpt-5.6-luna",
    toolScope: "triage",
  ...
  check(
    "a spawn under the ceiling keeps its capability",
    under.capability === "cheap",
    JSON.stringify(under),
  );
  check(
    "a spawn under the ceiling keeps its model",
    under.model === "gpt-5.6-luna",
    JSON.stringify(under),
  );
  check(
    "a spawn under the ceiling reports nothing scrubbed",
    under.escalationScrubbed === undefined,
    JSON.stringify(under),
  );
  ...
  check(
    "a window-suffixed model is judged by the named-model arm",
    suffixed.capability === "balanced",
    JSON.stringify(suffixed),
  );
  check(
    "the suffixed model did not survive the clamp",
    suffixed.model !== "opus[1m]",
    JSON.stringify(suffixed),
  );
  check(
    "its replacement is what the PERMITTED capability resolves to",
    suffixed.model === "sonnet",
    JSON.stringify(suffixed),
  );
  check(
    "the refused 1M request did not ride along onto the replacement",
    !String(suffixed.model ?? "").includes("[1m]"),
    JSON.stringify(suffixed),
  );
  check(
    "the suffixed clamp is named in escalationScrubbed too",
    Array.isArray(suffixed.escalationScrubbed) &&
      suffixed.escalationScrubbed.includes("model"),
    JSON.stringify(suffixed),
  );
  ...
  try {
    ...
    check(
      "a SYMLINK to the capped directory does not walk around its ceiling",
      viaLink.capability === "balanced" && viaLink.model === "sonnet",
      JSON.stringify(viaLink),
    );
    check(
      "the symlinked spawn also keeps inert toolScope",
      viaLink.toolScope === "operator",
      JSON.stringify(viaLink),
    );
  } catch (err) {
    check(
      "symlink case ran",
      false,
      `could not create ${link}: ${err?.message ?? err}`,
    );
  }
  ...
  check(
    "the SAME request outside the capped tree is admitted",
    outside.capability === "frontier_plus" && outside.model === "fable",
  ...
  check(
    "an admitted model reaches the provider carrying its window request",
    kept.model === "opus[1m]",
    JSON.stringify(kept),
  );
  check(
    "and the admitted spawn reports nothing scrubbed",
    kept.escalationScrubbed === undefined,
    JSON.stringify(kept),
  );
  ...
  check(
    "a reviewer asking to RESUME a session is refused",
    typeof reviewerResume.err === "string",
    JSON.stringify(reviewerResume),
  );
  check(
  ...
  check(
    "the refused reviewer never reached the provider",
    !reviewerResume.reached,
    JSON.stringify(reviewerResume),
  );
  ...
  const capResume = await refused({
    cwd: tmp,
    capability: "deep_reviewer",
    resumeSessionId: "sess-2",
  });
  check(
    "a fresh CAPABILITY declared without a role is refused too",
    typeof capResume.err === "string",
    JSON.stringify(capResume),
  );
  ...
  check(
    "the freshness refusal fires inside a capped directory as well",
    typeof bothRefused.err === "string",
    JSON.stringify(bothRefused),
  );
  ...
  await operator.call("agents.spawn", {
    cwd: tmp,
    role: "implementer",
    resumeSessionId: "sess-ok-1",
  });
  const ordinary = seen[seen.length - 1] ?? {};
  check(
    "an implementer keeps its resume",
    ordinary.resumeSessionId === "sess-ok-1",
    JSON.stringify(ordinary),
  );

  await operator.call("agents.spawn", {
    cwd: tmp,
    resumeSessionId: "sess-ok-2",
  });
  const unlabelled = seen[seen.length - 1] ?? {};
  check(
    "a spawn declaring no role and no capability keeps its resume",
    unlabelled.resumeSessionId === "sess-ok-2",
  ...
  check(
    "a reviewer starting a NEW session is admitted",
    freshStart.role === "reviewer" && freshStart.capability === "deep_reviewer",
  ...
  const joined = await waitFor(async () => {
    ...
    return (
      rows.find((r) => r.kind === "spawn" && r.decisionId === decisionId) ??
      null
    );
  }, 5000);
  check(
    "the clamped spawn is appended to the decision log",
    !!joined,
    `no spawn row for ${decisionId}`,
  );
  if (joined) {
    check(
      "the spawn row records the ceiling that matched",
      joined.spawn?.ceiling?.key === CEILED_DIR,
      JSON.stringify(joined.spawn?.ceiling),
    );
    check(
      "the spawn row records what was taken",
      Array.isArray(joined.spawn?.scrubbed) &&
        joined.spawn.scrubbed.includes("model"),
      JSON.stringify(joined.spawn),
    );
    check(
      "the spawn row records the CANONICAL cwd the ceiling was looked up on",
      joined.spawn?.cwd === CEILED_DIR,
      JSON.stringify(joined.spawn),
    );
    check(
      "the spawn row records the caller tier, not its token",
      joined.spawn?.callerScope === "operator" &&
        !JSON.stringify(joined.spawn).includes(OPERATOR_TOKEN),
      JSON.stringify(joined.spawn),
    );
  }
...
async function runSlice2RoutingAssertions(caller, fakeURL) {
      ...
      async () =>
        caller.events
          ...
          .find(
            (e) =>
              e?.type === "routing.decision" &&
              e?.data?.decisionId === result?.decisionId,
          ),
      5000,
    );
  ...
  check(
    "codex-red: the answer falls over to the claude alternative",
    red.result?.provider === "claude" && red.result?.model === "opus",
    JSON.stringify(red.result),
  );
  check(
    "codex-red: the capability is unchanged — a fallover moves the PROVIDER",
    red.result?.capability === "frontier",
    JSON.stringify(red.result?.capability),
  );
  check(
    "codex-red: the subject capacity is still codex's own RED reading",
    red.result?.capacity?.provider === "codex" &&
      red.result?.capacity?.health === "red",
    JSON.stringify(red.result?.capacity?.health),
  );
  check(
    "codex-red: the event names the primary it passed over",
    red.event?.fellOverFrom?.provider === "codex",
    JSON.stringify(red.event?.fellOverFrom),
  );
  check(
    "codex-red: the event health describes CLAUDE, the provider about to run it",
    red.event?.health === "green" && red.event?.provider === "claude",
    JSON.stringify({
      health: red.event?.health,
      provider: red.event?.provider,
    }),
  );
  ...
  check(
    "codex-red: conserve steps the landing row down one rung",
    red.result?.effort === "medium",
    JSON.stringify(red.result?.effort),
  );
  check(
    "codex-red: the step is reported as a structured from/to",
    red.result?.effortStep?.from === "high" &&
      red.result?.effortStep?.to === "medium",
    JSON.stringify(red.result?.effortStep),
  );
  check(
    "codex-red: the step rides the routing.decision event",
    red.event?.effortStep?.to === "medium" && red.event?.effort === "medium",
    JSON.stringify(red.event?.effortStep),
  );
  check(
    "codex-red: the step is explained, not merely applied",
    /steps down the claude ladder/.test(red.result?.effortStep?.why ?? ""),
    red.result?.effortStep?.why ?? "",
  );
  ...
  check(
    "codex-red: a balanced tier is NOT stepped",
    balanced.result?.capability === "balanced" &&
      balanced.result?.effort === "high",
    JSON.stringify({
      capability: balanced.result?.capability,
      effort: balanced.result?.effort,
    }),
  );
  check(
    "codex-red: and it says why it was left alone",
    /effort_step_capabilities/.test(balanced.result?.effortStep?.why ?? ""),
    balanced.result?.effortStep?.why ?? "",
  );
  ...
  check(
    "skipped-middle: the walk lands on the third candidate",
    skipped.result?.provider === "copilot" &&
      skipped.result?.model === "gpt-5-mini",
    JSON.stringify(skipped.result),
  );
  check(
    "skipped-middle: the disabled middle entry is named in the reasons",
    /alternative codex gpt-5\.6-sol[^|]*was skipped too: the entry is explicitly disabled/.test(
      reasonsOf(skipped.result),
    ),
    reasonsOf(skipped.result),
  );
  check(
    "skipped-middle: the primary it fell over FROM is the claude one",
    skipped.result?.fellOverFrom?.provider === "claude",
    JSON.stringify(skipped.result?.fellOverFrom),
  );
  ...
  const pinned = await ask("healthy-current", {
    role: "implementer",
    provider: "claude",
    preferredProvider: "claude",
  });
  check(
    "pinned-provider: the answer is on the provider that was asked for",
    pinned.result?.provider === "claude",
    JSON.stringify(pinned.result),
  );
  check(
    "pinned-provider: it did NOT borrow another profile's pairing",
    !/took the pairing from profile/.test(reasonsOf(pinned.result)),
    reasonsOf(pinned.result),
  );
  ...
  const independent = await ask("healthy-current", {
    ...
    requireIndependentFamily: true,
  });
  check(
    "independent-family: the reviewer lands on a different family from the implementer",
    independent.result?.provider === "claude" &&
      independent.result?.independentFamily === true,
    JSON.stringify({
      provider: independent.result?.provider,
      independentFamily: independent.result?.independentFamily,
    }),
  );
  check(
    "independent-family: the walk says it tried the independent candidates first",
    /prefers a different one first/.test(reasonsOf(independent.result)),
    reasonsOf(independent.result),
  );
  ...
  check(
    "same-family fallback: a conserving claude sends the reviewer back to codex",
    sameFamily.result?.provider === "codex",
    JSON.stringify(sameFamily.result),
  );
  check(
    "same-family fallback: independence is reported HONESTLY as lost",
    sameFamily.result?.independentFamily === false,
    JSON.stringify(sameFamily.result?.independentFamily),
  );
  check(
    "same-family fallback: and the answer says it could not be arranged",
    /independent family was REQUIRED and could not be arranged/.test(
      reasonsOf(sameFamily.result),
    ),
    reasonsOf(sameFamily.result),
  );
  check(
    "same-family fallback: `fresh: true` is what still carries independence",
    sameFamily.result?.fresh === true,
    JSON.stringify(sameFamily.result?.fresh),
  );

  // 6. SPEND_DOWN STEPS UP, and stops at two different limits.
  const spend = await ask("healthy-current", {
    role: "supervisor",
    provider: "codex",
    preferredProvider: "codex",
  });
  check(
    "spend_down: the mode is actually spend_down",
    spend.result?.mode === "spend_down",
    JSON.stringify(spend.result?.reason),
  );
  check(
    "spend_down: an unpromoted role steps UP one rung",
    spend.result?.effort === "xhigh" &&
      spend.result?.effortStep?.from === "high",
    JSON.stringify(spend.result?.effortStep),
  );

  const promoted = await ask("healthy-current", {
    role: "implementer",
    provider: "codex",
    preferredProvider: "codex",
  });
  check(
    "spend_down: a role whose CAPABILITY was already promoted is not promoted twice",
    promoted.result?.capability === "frontier_max" &&
      promoted.result?.effort === "xhigh",
    JSON.stringify({
      capability: promoted.result?.capability,
      effort: promoted.result?.effort,
    }),
  );
  check(
    "spend_down: and the cap is explained",
    /one promotion per decision/.test(promoted.result?.effortStep?.why ?? ""),
    promoted.result?.effortStep?.why ?? "",
  );

  const clamped = await ask("healthy-current", {
    role: "judge",
    provider: "codex",
    preferredProvider: "codex",
  });
  check(
    "spend_down: a step off the top of the ladder CLAMPS",
    clamped.result?.effort === "xhigh",
    JSON.stringify(clamped.result?.effort),
  );
  check(
    "spend_down: and says the ladder ran out",
    /clamped at codex's ceiling/.test(clamped.result?.effortStep?.why ?? ""),
    clamped.result?.effortStep?.why ?? "",
  );
  ...
  check(
    "composed: the mode came from the conserving claude subject",
    composed.result?.mode === "conserve" &&
      composed.result?.capacity?.provider === "claude",
    JSON.stringify({
      mode: composed.result?.mode,
      subject: composed.result?.capacity?.provider,
    }),
  );
  check(
    "composed: the mode shift moved the capability across providers",
    composed.result?.baseCapability === "frontier_plus" &&
      composed.result?.capability === "cheap",
    JSON.stringify({
      base: composed.result?.baseCapability,
      capability: composed.result?.capability,
    }),
  );
  check(
    "composed: the walk then ran on top of the shift and landed elsewhere again",
    composed.result?.provider === "copilot" &&
      composed.result?.model === "gpt-5-mini",
    JSON.stringify(composed.result),
  );
  check(
    "composed: fellOverFrom names the row the WALK passed over",
    composed.result?.fellOverFrom?.provider === "codex",
    JSON.stringify(composed.result?.fellOverFrom),
  );
  check(
    "composed: shiftCapacity describes the FINAL landing, not the shift's",
    composed.result?.shiftCapacity?.provider === "copilot",
    JSON.stringify(composed.result?.shiftCapacity?.provider),
  );
  check(
    "composed: the event names the landing provider and its own health",
    composed.event?.provider === "copilot" &&
      composed.event?.health === composed.result?.shiftCapacity?.health,
    JSON.stringify({
      provider: composed.event?.provider,
      health: composed.event?.health,
    }),
  );
}
...
async function runAvailabilityAssertions(caller, fakeURL) {
  ...
  check(
    "availability: an UNKNOWN provider still gets the work (fail open)",
    before?.provider === "codex",
    JSON.stringify({ provider: before?.provider, reason: reasonsOf(before) }),
  );
  ...
  const reprobed = await waitFor(async () => {
    await caller.call("routing.select", {
      ...routingRequest("availability-kick", ""),
      provider: "",
      preferredProvider: "",
      role: "implementer",
    });
  ...
  check(
    "availability: a decision kicks a background re-probe of the provider catalog",
    !!reprobed,
    `codex model probes stayed at ${probesBefore}`,
  );
  ...
  check(
    "availability: a provider that reports no launchable model is routed AROUND",
    !!after,
    "the decision stayed on codex after its catalog went empty",
  );
  if (after) {
    check(
      "availability: the reason names the live probe, not the allowance",
      /is not available to launch right now/.test(reasonsOf(after)),
      reasonsOf(after),
    );
    check(
      "availability: the reason says what the probe SAW and does not claim a missing CLI",
      /ran and reported no launchable model/.test(reasonsOf(after)) &&
        !/not installed/.test(reasonsOf(after)),
      reasonsOf(after),
    );
    check(
      "availability: and the allowance it skipped was healthy, so nothing else explains the move",
      after?.capacity?.health === "green",
      JSON.stringify(after?.capacity?.health),
    );
  }
  ...
  check(
    "availability: a provider that starts answering again is routable again inside the catalog TTL",
    !!restored,
    "codex stayed unavailable after its catalog was restored — the forced refresh is not re-probing unavailable entries",
  );
  if (restored) {
    check(
      "availability: and the restored answer carries no fallover at all",
      !restored?.fellOverFrom,
      JSON.stringify(restored?.fellOverFrom),
    );
  }
}
...
try {
  ...
  execFileSync("go", ["build", "-o", hubBin, "./cmd/hub"], {
    cwd: hubDir,
    stdio: "inherit",
  });
  ...
  check(
    "healthy-current: Codex 5h percentage is a real ok scalar",
    five?.used_percent?.state === "ok" && five.used_percent.value === 12.0,
    JSON.stringify(five),
  );
  check(
    "healthy-current: Codex 5h reset is in the future",
    five?.resets_at > report.generated_at && five?.is_current === true,
    JSON.stringify(five),
  );
  ...
  check(
    "stale-codex: captured used_percent is verbatim 67.0",
    five?.used_percent?.state === "ok" && five.used_percent.value === 67.0,
    JSON.stringify(five),
  );
  check(
    "stale-codex: captured resets_at is verbatim 1787953526",
    five?.resets_at === 1787953526,
    JSON.stringify(five),
  );
  check(
    "stale-codex: captured window_minutes is verbatim 300",
    five?.window_minutes === 300,
    JSON.stringify(five),
  );
  check(
    "stale-codex: fake marks the rolled window non-current",
    five?.is_current === false,
    JSON.stringify(five),
  );
  ...
  five = codexFive(report);
  check(
    "reset-now: resets_at equals generated_at exactly",
    five?.resets_at === report.generated_at,
    JSON.stringify({ generated_at: report.generated_at, five }),
  );
  check(
    "reset-now: equality is non-current",
    five?.is_current === false,
    JSON.stringify(five),
  );
  ...
  check(
    "copilot-403: Copilot account is present",
    !!copilot?.accounts?.[0],
  ...
  check(
    ...
    copilotWindows.length === 3 &&
      copilotWindows.every(
        (w) =>
          w.used_percent?.state === "unavailable" &&
          /403/.test(w.used_percent?.reason ?? ""),
      ),
  ...
  check(
    "absent-opencode-pi: opencode is absent from the usage report",
    !names.includes("opencode"),
    JSON.stringify(names),
  );
  check(
    "absent-opencode-pi: pi is absent from the usage report",
    !names.includes("pi"),
    JSON.stringify(names),
  );
