...
func TestMainOwnedCapabilitiesDoNotCollideWithTheBrain(t *testing.T) {
	...
	declaredOverlap := map[string]overlap{
		...
		"claude.setModel":          {true, "both POST the canonical pair plus legacy companion to claudemon /sessions/:id/model, whose accepted switch updates its owner-authored requested_selection and durable dual-write; the desktop also writes the accepted result into Electron's local mirror immediately, while the adopted brain waits for the owner snapshot broadcast"},
