...
func TestMainOwnedCapabilitiesDoNotCollideWithTheBrain(t *testing.T) {
	...
	declaredOverlap := map[string]overlap{
		...
		"fleetWorkflows.request": {true, "Desktop-only v1. The production catalog-only brain does not register it; a full headless brain explicitly returns unavailable. Never advertises runtime parity or launches workflows."},
