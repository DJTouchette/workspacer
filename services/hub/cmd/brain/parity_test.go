...
var spawnParamsDeclined = map[string]string{
	...
	"resultSchema":    "the structured-result contract has TWO halves and the FIRST is desktop-owned: the schema is compiled into the worker's spawn prompt (claudeSpawn's --append-system-prompt / managedSpawn's first-turn instructions), which the brain does not do, so a headless worker is never even asked for a wks-result block. The brain now HAS the delivery half (finishwake.go's worker-finished wake, which the desktop's supervisorNudge used to own alone), but a validated object needs something to validate — so a brain-answered spawn would still take the param and silently never honor it",
