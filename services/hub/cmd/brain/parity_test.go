...
var spawnParamsDeclined = map[string]string{
	...
	"fleetFullAccess": "legacy compatibility only: supported agents receive ambient Workspacer tools and Workspacer does not consult or rewrite token grant fields; provider permission mode is passed through independently",
	...
	"resultSchema":    "caller-authored resultSchema needs the desktop's JSON-Schema-subset validator at finish. The brain now injects fixed host-authored instructions (including worker escalation) on both PTY and managed paths, but it still has no general validator for an arbitrary schema — accepting this param would silently leave the wks-result half unvalidated",
