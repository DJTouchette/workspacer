	...
	seeds := []libraryItem{
		...
		// framing is canned.
		{
			...
			Body: strings.Join([]string{
				...
				"- If the task above includes a scout’s verified handoff, treat it as your baseline: read the files and entry points it names and start building rather than re-investigating from scratch. Reopen it only on a concrete contradiction, new evidence, or a gap it explicitly marked unresolved — and say why.",
				"",
				...
				"",
				"So end your turn with a short, factual HANDOFF: the actual commit and files changed, the acceptance criteria and architectural constraints, the files a reviewer should read first, checks with their results, the relevant environment, and remaining uncertainty or unmet criteria. Preserve the required wks-result and wks-escalation contracts; do not turn this into a universal long report. Leave your plan and your reasoning out of it; they would anchor the reviewer on the thinking that produced the code. That final message reaches your manager automatically; do not try to message anyone, just finish.",
			}, "\n"),
		},
		{
			...
			Body: strings.Join([]string{
				...
				"{{handoff}}",
				"",
				"Read the diff against the criteria. {{focus:Weigh correctness first, then whether the change fits the code around it, then test coverage, then the rest.}} The implementer’s checks and their logged output are context, not proof of correctness — weigh them, do not inherit them. Treat scout conclusions as context too, not evidence. Verify independently rather than take a claim on trust: pick the checks that matter for the risk in this diff and re-run those yourself instead of re-running everything by default. Repeat a full suite when the diff touches shared, concurrent, or security-sensitive code, when your environment or the integration path differs from what the implementer ran, when a check failed or its evidence is missing or looks unreliable, or when this repo’s own policy calls for it here. Keep track of which conclusions you established by RUNNING something yourself and which you only read.",
				"",
		...
		},
		{
			...
			Body: strings.Join([]string{
				...
				"{{task}}",
				"",
				"Your handoff is what lets the implementer skip re-discovering what you already found, so make it usable without a re-investigation: name the exact files and symbols involved, state only facts you verified rather than guessed, propose a contract or acceptance criteria for the work, and mark every unknown explicitly as an unknown rather than leaving it implied.",
				"",
