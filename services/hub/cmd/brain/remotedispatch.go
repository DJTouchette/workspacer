...
func (r *registry) dispatchCapabilities(ctx context.Context, raw json.RawMessage) (json.RawMessage, error) {
	var caller struct {
		OriginKey string `json:"originKey"`
	}
	if len(raw) > 0 {
		if err := unmarshal(raw, &caller); err != nil {
			return nil, err
		}
	}
	out := map[string]any{
		"protocol": bus.DispatchProtocol,
		"handoff":  map[string]any{"version": 1, "transport": "git-remote", "ownerKey": caller.OriginKey, "objectFormats": []string{"sha1", "sha256"}, "chunkBytes": 256 << 10, "fileBytes": 16 << 20, "taskBytes": 64 << 20, "files": 128},
