...
// Session liveness still matters for UI and lifecycle bookkeeping even though
// it no longer widens an agent's filesystem authority.
func TestSnapshotLivenessCases(t *testing.T) {
	cases := []struct {
		name     string
		snapshot json.RawMessage
		live     bool
	}{
		{"active status", json.RawMessage(`{"cwd":"/w/p","status":"active"}`), true},
		{"no state yet", json.RawMessage(`{"cwd":"/w/p"}`), true},
		{"ended status", json.RawMessage(`{"cwd":"/w/p","status":"ended"}`), false},
		{"stopped mode", json.RawMessage(`{"cwd":"/w/p","mode":"stopped"}`), false},
		{"archived", json.RawMessage(`{"cwd":"/w/p","mode":"running","archived":true}`), false},
		{"terminal unknown mode", json.RawMessage(`{"cwd":"/","mode":"unknown"}`), false},
		{"running mode wins stale status", json.RawMessage(`{"cwd":"/w/p","mode":"running","status":"ended"}`), true},
		{"running row without cwd", json.RawMessage(`{"mode":"running"}`), true},
		{"invalid row", json.RawMessage(`"broken"`), false},
	}
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			if got := snapshotLive(c.snapshot); got != c.live {
				t.Errorf("snapshotLive(%s) = %v, want %v", c.snapshot, got, c.live)
			}
		})
	}
}
