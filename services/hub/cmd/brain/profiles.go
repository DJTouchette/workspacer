...
// remoteSpawnProfile resolves a provider profile from this host's own store.
// Authenticated spawns may select profiles ambiently; the boolean remains only
// for mixed-version call sites and has no policy effect.
