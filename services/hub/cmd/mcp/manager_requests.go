...
type managerRequestIn struct {
	...
	Intents              []map[string]any `json:"intents,omitempty" jsonschema:"1-8 independent actions with stable key, kind(create/followUp/update/question/none/untracked), reason; create/followUp adds cwd, title, provenance(explicit/inferred), optional workflowId(null=no workflow, id=chosen policy, omitted=default); untracked records an explicit opt-out with key/kind/reason only and creates no task; update requires taskId and expectedTaskRevision; workflowId may change only before the first worker; followUp requires dependsOn task IDs or dependsOnKeys naming earlier work intents in this same resolution; cancel is optional on update; references optionally maps pasted URLs using kind pullRequest(number,url), ticket(id,url), or reference(label,url); multiple URLs/tasks require references on every work intent ([] assigns none); replacePullRequest true requires explicit replacement intent"`
