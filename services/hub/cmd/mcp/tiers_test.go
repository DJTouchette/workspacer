...
// Deferral saves context only when loading the current action's schemas.
func TestHelpBatchesOnlyCurrentTurnSchemas(t *testing.T) {
	for _, scope := range []authtoken.Scope{authtoken.ScopeView, authtoken.ScopeOperator} {
		infos := toolInfosFor(t, scope)
		hint := batchLoadHint(infos)
		if !strings.Contains(hint, "only the tools needed for this turn") || !strings.Contains(hint, "Do not preload") {
			t.Fatalf("unexpected guidance: %s", hint)
		}
		for _, tool := range infos {
			if strings.Contains(hint, tool.Name) {
				t.Fatalf("hint eagerly names %s", tool.Name)
			}
		}
	}
}
