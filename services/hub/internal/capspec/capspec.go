...
var unscopedByDecision = map[string]string{
	...
	// nodes.wake — start a remote node that is not running. Registered by
	// cmd/hub only when a nodes.json exists.
	...
	// nodes.sleep — stop a running remote node. The other half of nodes.wake,
	// registered by cmd/hub only when a nodes.json exists.
	...
	"claude.setPermissionMode": "mode is provider permission configuration, not a path. Authenticated agents may select any provider-supported mode without a second Workspacer grant",
	...
	// usage.setPacingSchedule — the one Overview pacing preference a client may
	// set. It is the write half of usage.pacingSchedule and it exists as its own
	// narrow file + method pair precisely BECAUSE routing.yaml has no write RPC:
	// see cmd/hub/main.go, where the absence of one is stated as the reason
	// routing.yaml's `ceilings:` block is a ceiling.
	"usage.setPacingSchedule": "the only caller value is `schedule`, a CLOSED two-word enum (five_day | seven_day) validated by usageprefs.Validate before anything is written; an unrecognised word is an error and stores nothing. It is never opened as a path, never becomes argv, and never reaches an interpreter: it SELECTS which of two curve words internal/limits already implements is used for the seven-day window of the next usage.report. There is no path to confine, so the gate is IDENTITY — usagePrefsTrusted(\"usage.setPacingSchedule\", …) refuses plugin tokens and the view/triage tiers, exactly as jobsTrusted does — and the method appears in no scoped tier. What it emphatically does NOT touch is routing: routing.select reads Matrix.PaceConfig() straight off routing.yaml and never consults this preference, so nothing here can move a spawn ceiling, a model choice or a routing mode",
...
var unscopedParams = map[string]map[string]ParamDecision{
	...
	"agents.spawn": {
		...
		"mcpItemIds":               {KindID, "selects Library MCP entries merged beside the automatic authenticated Workspacer facade. Holding agents.spawn is the trust boundary; Workspacer no longer applies a second per-session grant"},
		"profileId":                {KindID, "selects a stored provider profile whose config root and provider arguments flow to the launched harness. Holding agents.spawn is the trust boundary; legacy profile grant stamps are ignored"},
		...
		"skipPermissions": {KindPermission, "provider permission configuration forwarded by an authenticated agents.spawn caller; no separate Workspacer grant or clamp"},
		"permissionMode":  {KindPermission, "provider-native permission mode forwarded by an authenticated agents.spawn caller; no separate Workspacer grant or clamp"},
	...
	"claude.setPermissionMode": {
		"mode": {KindPermission, "selects the running provider's supported permission mode; authenticated agent access is the trust boundary and Workspacer applies no separate grant"},
	},
	"claude.setEffort": {
	...
	"library.save": {
		...
		"command": {KindExecutable, "argv[0] of a Library MCP server. It executes only when an authenticated agents.spawn caller explicitly selects that item; the spawn capability is the trust boundary"},
