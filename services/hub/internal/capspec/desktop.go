...
func init() {
	...
	compositionInert["ui.asset"] = InertClaim{Reason: "The file selects cached display bytes under a fixed root. The semantic assertPathContained call confines it before descriptor access; no durable state or authority is changed, and the browser only interprets these bytes as a font or image.", Witnesses: []Witness{guarded(argBearing("assertPathContained", "ui.asset", []string{"apps", "desktop", "src", "main", "headless", "uiAssets.ts"}))}}
