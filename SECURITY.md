...
The guards those findings produced are still in the code, and each one now
explains itself where it sits — no comment depends on this file to make sense.
If you want the original write-ups (threat model, evidence, and what was decided
for each of the sixteen), they are one command away:
